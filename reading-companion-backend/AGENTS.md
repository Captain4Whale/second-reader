...
- Treat `../contract/annotation-pack/v0/README.md#producer-neutral-information-responsibility` as the stable adapter target. A mechanism-specific version such as `attentional_v2-phase9` may gate one adapter but must never become a required neutral-draft field or be copied into a new mechanism. Adapters supply `kind`, exact shared-source range/quote, conditional Note text, and settlement time; source verification and public-wire derivation remain generic.
...
- After `DEC-105`, retired Detour / source-backread / source-skill compatibility interfaces are not a current `attentional_v2` code, prompt, schema, audit, or test surface.
