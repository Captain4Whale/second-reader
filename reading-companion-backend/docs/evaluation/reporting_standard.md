...
- When showing Active Attention, show the concrete live-inquiry fields (`question_from`, `driving_question`, `answer_boundary`, `working_answer`, `source_refs`, `answer_source_refs`, `status`) rather than rewriting it as a generic memory summary. Do not require `driving_question` to be a literal question-mark sentence; it may be a tension or watchpoint, but it must have one answer boundary. Label old `statement`-only entries as legacy artifacts.
