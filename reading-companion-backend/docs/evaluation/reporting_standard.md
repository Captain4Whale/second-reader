...
- When showing Active Attention, show the concrete live-inquiry fields (`question_from`, `driving_question`, `working_answer`, `source_refs`, `answer_source_refs`, `status`) rather than rewriting it as a generic memory summary. Do not require `driving_question` to be a literal question-mark sentence; it may be a coherent reading tension, suspense, or watchpoint. It may be grounded in prompt-visible source, title / chapter framing, or prior memory, but reports should call out any book / author / later-chapter knowledge that was not visible in the prompt. For terminal items, show `answered_reason` or `closed_reason` plus opened / answered / closed source coordinates when available. Label old `statement`-only entries and `answer_boundary` fields as legacy artifacts rather than current required structure.
