...
- When showing Active Attention, show the concrete live-inquiry fields (`question_from`, `driving_question`, `working_answer`, `source_refs`, `answer_source_refs`, `status`) rather than rewriting it as a generic memory summary. Do not require `driving_question` to be a literal question-mark sentence; it may be a coherent reading tension, suspense, or watchpoint. For terminal items, show `answered_reason` or `closed_reason` plus opened / answered / closed source coordinates when available. Label old `statement`-only entries and `answer_boundary` fields as legacy artifacts rather than current required structure.
- When showing Active Attention source evidence, distinguish deterministic coordinate resolution (`matched`, `ambiguous_first_match`, normalized exact match, ordered-fragment match) from `fallback_unit_span` caveats. A fallback span is useful diagnostic context, not precise source evidence.
