...
- When showing Active Attention, show the concrete live-question fields (`question_from`, `driving_question`, `working_answer`, `source_refs`, `answer_source_refs`, `status`) rather than rewriting it as a generic memory summary. Label old `statement`-only entries as legacy artifacts.
