...
- When showing Active Attention, show it as ActiveTension with the concrete fields (`tension_from`, `tension_focus`, `working_interpretation`, `source_refs`, `development_source_refs`, `status`) rather than rewriting it as a generic memory summary. Do not require `tension_focus` to be a literal question; it may be a question, suspense, image, beauty, character trait, unusual event, emotional pressure, pattern, or watchpoint. It may be grounded in prompt-visible source, title / chapter framing, or prior memory, but reports should call out any book / author / later-chapter knowledge that was not visible in the prompt. For terminal items, show `answered_reason` or `closed_reason` plus opened / answered / closed source coordinates when available. Label old `question_from`, `driving_question`, `working_answer`, `answer_source_refs`, `statement`, and `answer_boundary` fields as migration inputs from old artifacts rather than current required structure.
