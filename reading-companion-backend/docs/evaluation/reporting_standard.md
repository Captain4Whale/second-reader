...
- When showing Recent Reading Memory, foreground `entry_id`, `source_unit_span_id`, `kind`, `memory_text`, `status`, `created_at_unit_index`, and `archived_by_consolidation_id`. Treat it as unit-level semantic memory: do not invent fine-grained source refs when the artifact only provides unit-span provenance, and do not foreground generated operation-level reasons as Recent Memory evidence. Review `memory_text` for source-established content first: it should record what the unit says, shows, names, contrasts, changes, withholds, or explicitly frames; author stance / evidence-boundary / intended-reader units should not be dismissed as empty structure; reports should flag unsupported abstract closing labels such as "this is a mechanism", "this forms a tension", or "this proves..." when the source did not explicitly name or frame that abstraction; and reports should also flag default `<label>: <explanation>` heading style when the source did not itself name that term, stage, or framework.
