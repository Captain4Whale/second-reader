...
- Memory Quality should score the complete probe-time memory state when it is available. The current state evidence is `scoring_memory_state` with `active_attention` (deprecated store), `concept_registry`, `thread_trace`, and `reflective_frames`; show empty arrays, empty objects, and `null` values explicitly rather than silently omitting them. Once `recent_reading_memory` exists, reports should treat it as the near-term memory evidence instead of extending `active_attention`.
...
- When showing Active Attention, label it as deprecated ActiveTension evidence. Existing runs may still expose concrete fields (`tension_from`, `tension_focus`, `working_interpretation`, `source_refs`, `development_source_refs`, `status`), and reports may explain them for diagnostic review, but should not present `active_attention` as the target near-term memory architecture. Future reports should foreground `recent_reading_memory` for near-term per-unit memory once it exists, and `thread_trace` for durable tensions / arcs / watchpoints. Label old `question_from`, `driving_question`, `working_answer`, `answer_source_refs`, `statement`, and `answer_boundary` fields as old artifact fields rather than current required structure.
