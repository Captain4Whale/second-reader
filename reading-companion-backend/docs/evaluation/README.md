...
| `Long Span active direction` | `—` | `attentional_v2` vs `iterator_v1` | `phase-1 implemented; no current formal run yet` | The active Long Span direction has moved away from `target-centered accumulation v2` and is now defined by `Memory Quality`, `Spontaneous Callback`, and `False Visible Integration`. Phase 1 is now implemented through `run_long_span_vnext.py`, but no formal long-span run has been promoted yet under the new direction. The April 22 rejudge and the April 19 rerun remain preserved diagnostic evidence for the discontinued target-centered route. | [methodology](../../../docs/backend-reader-evaluation.md) · [surface index](./long_span/README.md) · [phase-1 runner](../../eval/attentional_v2/run_long_span_vnext.py) · [archived target-centered design](./long_span/target_centered_accumulation_v2_design.md) · [April 22 diagnostic aggregate](../../eval/runs/attentional_v2/attentional_v2_accumulation_benchmark_v2_frozen_rejudge_contract_fix_20260422/summary/aggregate.json) · [April 22 diagnostic report](../../eval/runs/attentional_v2/attentional_v2_accumulation_benchmark_v2_frozen_rejudge_contract_fix_20260422/summary/report.md) |
