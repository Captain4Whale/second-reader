...
def test_ingest_no_prior_unit_memory_tool_result_drops_bad_recall_args(tmp_path: Path, monkeypatch):
    ...
    def fake_tool_loop(system_prompt, prompt, *, action_tools, output_tool, tool_handler, validator, max_tool_calls):
        tool_args = {
            ...
            "memory_recalls": [
                {
                    "recall_id": "r1",
                    "recall_text": "Earlier reading about Siddhartha's awakening.",
                    "basis": "selected_source_unit",
                }
            ],
        }
        ...
        payload = {
            "unit": _unit(1, "乔文达仍然跟随他。"),
            "preview_partition": [_partition("乔文达跟随", 1, "乔文达仍然跟随他。")],
            "reason": "乔文达跟随悉达多离开家庭。",
        }
        assert validator(payload, [{"name": "retrieve_unit_memory", "args": tool_args, "result": tool_result}]) == []
        return SimpleNamespace(
    ...
    result = ingest(
        current_view_position={"current_chapter_id": 1, "current_cursor": {"paragraph_index": 1, "char_offset": 0}},
        current_view_content={"paragraph_slices": [{"paragraph_index": 1, "text": "乔文达仍然跟随他。"}]},
        output_dir=tmp_path,
        unit_memory_tool_handler=lambda _args: {
    ...
    assert captured["tool_result"]["status"] == "no_prior_unit_memory"
    assert result["memory_recalls"] == []
    assert result["memory_recalls_status"] == "no_prior_unit_memory"
    assert result["tool_loop_status"] == "tool_called"
...
def test_digest_uses_live_xml_prompt_and_filters_surface_reactions(tmp_path: Path, monkeypatch):
    ...
    assert '"marginalia_audit": [' not in captured["prompt"]
    ...
    assert "local facts, plot evidence, scene details" in captured["prompt"]
    assert "ordinary informative sentences" in captured["prompt"]
    ...
    assert "must not be fragmentary" in captured["prompt"]
    assert "这个人是神圣的" in captured["prompt"]
    assert "那天晚上，城门口多了七具尸体。" in captured["prompt"]
    assert "买下梵高画作的日本人" in captured["prompt"]
    assert "Japanese asset bubble" in captured["prompt"]
    ...
    assert "marginalia_audit" not in result
    ...
    assert manifest["prompt_version"] == "attentional_v2.digest.v19"
    assert manifest["prompt_assembly"]["spec_id"] == "attentional_v2.digest.xml.v19"
    ...
    assert validate_digest_result(
        {
            ...
            "marginalia_audit": [
                {
                    "source_quote": "Alpha hinge.",
                    "selection_reason": "Legacy audit reason can still repair old payloads.",
                }
            ],
        },
        current_unit_texts=["Alpha hinge."],
