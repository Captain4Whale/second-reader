...
def test_prompt_assembler_renders_spec_and_metadata_without_live_migration() -> None:
    ...
    assert DIGEST_PROMPT_VERSION == "attentional_v2.digest.v20"
...
def test_full_digest_prompt_xml_assembly_renders_all_live_blocks() -> None:
    ...
    assert result.spec_id == "attentional_v2.digest.xml.v20"
    ...
    assert result.output_contract == "digest_understanding_response_marginalia_json_v7"
...
def test_digest_reader_role_and_instruction_xml_renders_target_structure() -> None:
    ...
    assert "Durable excerpt value" in rendered
    assert "little long-term value as a standalone excerpt" in rendered
    assert "Do not select only the final conclusion" in rendered
    ...
    assert '"Smallest complete" does not mean "shortest possible sentence."' in rendered
    ...
    assert "valuable thought nodes" in rendered
    assert "Do not treat Marginalia as a top-1 or top-2 selection task" in rendered
    assert "Which exact source spans in this unit genuinely deserve to be carried forward?" in rendered
    assert "## Seperate or together?" in rendered
    assert "Before emitting adjacent Marginalia items" in rendered
...
def test_digest_output_contract_xml_renders_target_contract() -> None:
    ...
    assert "Highlight-only" in rendered
    assert "Note-bearing" in rendered
    ...
    assert "names both out-of-context completeness and durable excerpt value" in rendered
...
def test_attentional_v2_prompt_registry_contains_node_definitions() -> None:
    ...
    assert all(
        definition.output_contract.endswith(("_v1", "_v2", "_v3", "_v4", "_v5", "_v6", "_v7"))
...
def test_attentional_v2_prompt_registry_projects_current_bundle() -> None:
    ...
    assert ATTENTIONAL_V2_PROMPTSET_VERSION == "attentional_v2-phase6-v80"
    assert ATTENTIONAL_V2_PROMPTS.promptset_version == ATTENTIONAL_V2_PROMPTSET_VERSION
    assert digest.version == DIGEST_PROMPT_VERSION == "attentional_v2.digest.v20"
...
def test_highlight_only_marginalia_persists_without_fabricated_note_text() -> None:
