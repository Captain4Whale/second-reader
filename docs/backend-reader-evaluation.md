...
- The first Phase-1 diagnostic run has been corrected in two steps:
  ...
  - Memory Quality probe snapshots should score complete probe-time memory state:
    ...
    - Active Attention should be interpreted as open live inquiries with `question_from`, `driving_question`, `answer_boundary`, `working_answer`, and source evidence; reports should not treat it as a generic recent-memory digest or summary cache, and should evaluate resolve/close decisions against the item's answer boundary
