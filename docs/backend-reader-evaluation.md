...
- The first Phase-1 diagnostic run has been corrected in two steps:
  ...
  - Memory Quality probe snapshots should score complete probe-time memory state:
    ...
    - Active Attention should be interpreted as open live inquiries or coherent reading tensions with `question_from`, `driving_question`, `working_answer`, source evidence, and terminal `answered_reason` / `closed_reason` when present; reports should not treat it as a generic recent-memory digest, a formal Q&A tracker, or a summary cache. These inquiries may be grounded in prompt-visible source, title / chapter framing, or prior memory, but should not import book / author / later-chapter knowledge that was not present in the prompt. Evaluate resolve/close decisions against the reason and source evidence given at closure time. `answer_boundary` is legacy compatibility metadata if present, not the current scoring/display contract.
    - Active Attention source coordinates are program-resolved from model-cited exact `source_quote` / `answer_source_quote`; reports should distinguish exact/normalized/ordered-fragment matches from `fallback_unit_span` grounding caveats. If an inquiry is grounded in title/framing/prior memory rather than a current-source phrase, reports should say so instead of treating unit coordinates as precise source evidence.
