...
- The first Phase-1 diagnostic run has been corrected in two steps:
  ...
  - Memory Quality evidence reports have a fixed writing contract:
    ...
    - `Recent Routes` should label route explanations as `route reason`, because they are projected from `route_history.routes[-3:]`, not generic memory `statement` fields
