...
The old Memory / Planning / Evaluation implementation guidance chain at `docs/implementation/new-reading-mechanism/second-reader-memory-planning/README.md` is historical/reference material after `DEC-103`, not the default authority for new mechanism work. In that workspace, `C设计9-Evaluation Calibration & Minimal Eval Suite v0.md` remains useful background for implementation-facing eval discipline; this file remains the stable evaluation constitution.
...
- The first Phase-1 diagnostic run has been corrected in two steps:
  ...
  - Memory Quality scoring now uses:
    ...
    - `5 = excellent`
    - higher is better
  - Memory Quality scoring also includes a structural-signal supplement:
    - when source-so-far explicitly introduces a stage model, classification, core definition, roadmap, or named distinction, the judge should check whether the snapshot retains that structure in some meaningful form
  ...
  - Memory Quality probe snapshots should score complete probe-time memory state:
    ...
    - `scoring_memory_state.unit_memory` or an equivalent Unit Memory review projection for current long-distance per-unit Understanding memory
    - prompt-facing `ReadingMemory` trace/projection evidence for what Digest actually received at the probe boundary when long-distance recall was attempted
    ...
    - Unit Memory retrieval-health review should distinguish prompt-visible hot source-span duplicate suppression from arbitrary recent-window suppression. After `DEC-153`, a nearby prior unit outside prompt-visible hot memory is eligible if relevance and selection quality are strong enough.
...
- When a non-judged focused quality audit is used ahead of a broader rerun, the audit summary should explicitly record:
  ...
  - silent-unit count
  - whether any removed non-mainline path fields appeared unexpectedly
