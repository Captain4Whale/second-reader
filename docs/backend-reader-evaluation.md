...
- The first Phase-1 diagnostic run has been corrected in two steps:
  ...
  - Memory Quality probe snapshots should score complete probe-time memory state:
    ...
    - Active Attention should be interpreted as ActiveTension: points that still hang in reader attention with `tension_from`, `tension_focus`, `working_interpretation`, source evidence, and terminal `answered_reason` / `closed_reason` when present. Reports should not treat it as a generic recent-memory digest, a formal Q&A tracker, or a summary cache. ActiveTension may be grounded in prompt-visible source, title / chapter framing, or prior memory, but should not import book / author / later-chapter knowledge that was not present in the prompt. Evaluate resolve/close decisions against the reason and source evidence given at closure time. Old `question_from` / `driving_question` / `working_answer` / `answer_source_refs` / `answer_boundary` fields are migration inputs if present, not the current scoring/display contract.
    - Active Attention source coordinates are program-resolved from model-cited exact `source_quote` / `development_source_quote`; reports should distinguish exact/normalized/ordered-fragment matches from `fallback_unit_span` grounding caveats. If an item is grounded in title/framing/prior memory rather than a current-source phrase, reports should say so instead of treating unit coordinates as precise source evidence.
