...
- The `Ingest -> Digest -> Reading Runner settlement` mechanism reframe is implemented; current work is fact alignment, smoke/diagnostic review, and calibration before any formal evaluation promotion.
  ...
  - live cleanup landed:
    ...
    - Digest prompt `attentional_v2.digest.v20` now frames `understanding` as concise source-established content from the current source text and frames Marginalia as page-margin reader marks anchored to exact source spans, not generic explanatory annotations; Marginalia may be highlight-only or note-bearing, highlight-only is reserved for quote-only excerpts that remain understandable when lifted out of the book and carry durable long-term reader value, each highlight-only item carries a private inline `selection_reason` naming both completeness and durable excerpt value, note-bearing content should share useful knowledge, context, or non-obvious connections that a thoughtful ordinary reader may not know, notice, or infer on their own, ordinary close-reading / technique notes are explicitly demoted unless they provide non-obvious gain, and dense units should preserve all genuinely durable thought nodes instead of forcing a top-1/top-2 choice; `source_quote` should be the smallest complete contiguous local meaning span, meaning the shortest contiguous span that preserves the full reusable idea rather than the shortest possible sentence; the active promptset bundle is `attentional_v2-phase6-v80`
    - subject-continuity / standalone Understanding follow-up remains implemented in Digest prompt `attentional_v2.digest.v20`: prior Understanding in `ReadingMemory` carries narrator / speaker / actor / concept continuity; Digest establishes new subjects, continues known subjects when supported, or explicitly preserves meaningful ambiguity without adding raw prior-source backfill, Ingest reference-resolution fields, or a durable referent store
