...
- The `Ingest -> Digest -> Reading Runner settlement` mechanism reframe is implemented; current work is fact alignment, smoke/diagnostic review, and calibration before any formal evaluation promotion.
  ...
  - next step:
    ...
    - use `docs/implementation/new-reading-mechanism/mechanism-pattern-ledger.md` entry 19, `DEC-116`, `DEC-117`, and `DEC-118` for the accepted design explanation of why window-partition Ingest improves next-unit selection, why live v15 now records `preview_partition[]` titles as mechanism-private audit metadata, and why runtime preview capacity is now character-bounded instead of paragraph-count bounded
