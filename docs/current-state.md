...
Last verified: `2026-06-07T08:51:54+08:00`
...
- The `Ingest -> Digest -> Reading Runner settlement` mechanism reframe is implemented; current work is fact alignment, smoke/diagnostic review, and calibration before any formal evaluation promotion.
  ...
  - live cleanup landed:
    ...
    - Ingest prompt `attentional_v2.ingest.v8` calibrates `RecallPriorReading` as retrieval-friendly prior-reading recall: it asks for focused memory intentions that name the earlier understanding runtime should try to find, looks backward beyond the selected unit rather than summarizing the selected unit itself, avoids paragraph / line / XML identifiers, keeps recall text in the current source text's primary language, preserves source-form names/terms when available, and keeps model-side recall `basis` exactly `selected_source_unit`
