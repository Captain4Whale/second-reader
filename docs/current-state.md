...
Last verified: `2026-06-13T10:53:06+08:00`
...
- The `Ingest -> Digest -> Reading Runner settlement` mechanism reframe is implemented; current work is fact alignment, smoke/diagnostic review, and calibration before any formal evaluation promotion.
  ...
  - live cleanup landed:
    ...
    - Runtime builds a larger paragraph-offset reading lookahead window for Ingest: default `reader_policy.unitize` preview values are `preview_soft_min_chars=3000`, `preview_hard_max_chars=7000`, and `max_lookahead_paragraphs=12`; the live output contract now expresses the chosen boundary as `unit.end_paragraph_n` plus `unit.end_at` and stores `preview_partition_audit[]` as derived audit metadata
    ...
    - Ingest prompt `attentional_v2.ingest.v15` / promptset `attentional_v2-phase6-v65` promotes the reviewed window-partition selector plus the preview-partition audit map: it asks Ingest to view the visible lookahead as consecutive coherent reading units, title each provisional unit, commit only the first unit, treat paragraph boundaries as cues rather than defaults, and express boundaries through a visible paragraph `n` plus `paragraph_end` or a paragraph-local exact tail quote; `RecallPriorReading`, `ContextUseGuide`, retrieval tool semantics, and recall validation stay unchanged from the prior Unit Memory design
  ...
  - next step:
    ...
    - use `docs/implementation/new-reading-mechanism/mechanism-pattern-ledger.md` entry 19, `DEC-116`, and `DEC-117` for the accepted design explanation of why window-partition Ingest improves next-unit selection and why live v15 now records `preview_partition[]` titles as mechanism-private audit metadata
