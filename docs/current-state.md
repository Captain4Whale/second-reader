...
- The `Ingest -> Digest -> Reading Runner settlement` mechanism reframe is implemented; current work is fact alignment, smoke/diagnostic review, and calibration before any formal evaluation promotion.
  ...
  - live cleanup landed:
    ...
    - current `llm_calls.ingest(...)` is the forward-only XML LLM boundary call: `unit.end_paragraph_n`, `unit.end_at`, optional boundary-rationale `reason`, and bounded `memory_recalls[]`
    ...
    - Ingest prompt `attentional_v2.ingest.v14` / promptset `attentional_v2-phase6-v64` promotes the reviewed window-partition selector: it asks Ingest to view the visible lookahead as consecutive coherent reading units, commit only the first unit, treat paragraph boundaries as cues rather than defaults, and express the boundary through a visible paragraph `n` plus `paragraph_end` or a paragraph-local exact tail quote; `RecallPriorReading`, `ContextUseGuide`, retrieval tool semantics, and recall validation stay unchanged from the prior Unit Memory design
    - the current prompt does not require a full `preview_partition[]` output; per-partition titles remain a future candidate for making whole-window planning/audit more explicit while keeping `preview_partition[0]` as the only authoritative runtime boundary
