...
Last verified: `2026-06-21T14:50:51+08:00`
...
- The `Ingest -> Digest -> Reading Runner settlement` mechanism reframe is implemented; current work is fact alignment, smoke/diagnostic review, and calibration before any formal evaluation promotion.
  ...
  - live cleanup landed:
    ...
    - `DEC-135` tightens live Ingest first-unit boundary selection: Ingest prompt `attentional_v2.ingest.v18` / XML spec v18 / promptset `attentional_v2-phase6-v75` keeps the same boundary contract but adds a same-local-function rule so adjacent setup, character construction, scene build, argument support, example chains, or emotional turns are not split merely because their smaller facets can be titled separately
    ...
    - Ingest prompt `attentional_v2.ingest.v18` / promptset `attentional_v2-phase6-v75` keeps the reviewed window-partition selector plus preview-partition audit map, tightens recall ownership, and adds the same-local-function boundary rule: final output no longer includes `memory_recalls[]`; only `unit` / `preview_partition[0]` receives the optional top-level boundary `reason`; later `preview_partition[]` entries must stay to compact titles, boundaries, and status
    ...
    - Digest prompt `attentional_v2.digest.v16` now frames `understanding` as concise source-established content from the current source text and frames Marginalia as page-margin reader marks anchored to exact source spans, not generic explanatory annotations; Marginalia may be highlight-only or note-bearing, highlight-only is reserved for intrinsically valuable excerpt-worthy quotes whose value is visible in the quoted words themselves, each highlight-only item carries a private inline `selection_reason`, and `source_quote` should be the smallest complete contiguous local meaning span; the active promptset bundle is `attentional_v2-phase6-v75`
