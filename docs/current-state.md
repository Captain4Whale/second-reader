...
Last verified: `2026-06-07T10:22:16+08:00`
...
- The `Ingest -> Digest -> Reading Runner settlement` mechanism reframe is implemented; current work is fact alignment, smoke/diagnostic review, and calibration before any formal evaluation promotion.
  ...
  - live cleanup landed:
    ...
    - Ingest tool-loop validation now requires final submitted `memory_recalls[]` to match the `retrieve_unit_memory` action-tool recalls when the tool was called, preventing trace/runtime divergence where the model retrieves on one recall set and finalizes another; runtime does not broadly auto-absorb opening punctuation or next-unit markers
