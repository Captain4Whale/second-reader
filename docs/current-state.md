...
- The `Ingest -> Digest -> Reading Runner settlement` mechanism reframe is implemented; current work is fact alignment, smoke/diagnostic review, and calibration before any formal evaluation promotion.
  ...
  - live cleanup landed:
    ...
    - Digest prompt `attentional_v2.digest.v22` now frames `understanding` as concise source-established content from the current source text and frames Marginalia as page-margin reader marks anchored to exact source spans, not generic explanatory annotations; Marginalia may be highlight-only or note-bearing, highlight-only is reserved for quote-only excerpts that pass durable portable cognitive gain, out-of-context-completeness, excerpt-necessity, and selection-reason tests, each highlight-only item carries a private inline `selection_reason` naming completeness, the specific portable cognitive gain, and why the quote is more than local evidence / scene importance / emotional force / moral shock / a strong fact from the current book, note-bearing content should share useful knowledge, context, or non-obvious connections that a thoughtful ordinary reader may not know, notice, or infer on their own, ordinary close-reading / technique notes are explicitly demoted unless they provide non-obvious gain, dense units should still preserve every source span that genuinely passes the gates rather than forcing a top-1/top-2 choice, and plain reality-facing claims remain eligible only when they crystallize a reusable insight, distinction, mechanism, warning, model, or self-correction; `source_quote` should be the smallest complete contiguous local meaning span, meaning the shortest contiguous span that preserves the full reusable idea rather than the shortest possible sentence; the active promptset bundle is `attentional_v2-phase6-v82`
    - subject-continuity / standalone Understanding follow-up remains implemented in Digest prompt `attentional_v2.digest.v22`: prior Understanding in `ReadingMemory` carries narrator / speaker / actor / concept continuity; Digest establishes new subjects, continues known subjects when supported, or explicitly preserves meaningful ambiguity without adding raw prior-source backfill, Ingest reference-resolution fields, or a durable referent store
