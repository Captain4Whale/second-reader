...
- The `Ingest -> Digest -> Reading Runner settlement` mechanism reframe is implemented; current work is fact alignment, smoke/diagnostic review, and calibration before any formal evaluation promotion.
  ...
  - live cleanup landed:
    ...
    - Digest prompt `attentional_v2.digest.v23` now frames `understanding` as concise source-established content from the current source text and frames Marginalia as page-margin reader marks anchored to exact source spans, not generic explanatory annotations; Marginalia include Highlights and Notes as two independent reader actions rather than a content-empty/content-bearing choice. Highlights preserve exact source text worth carrying forward by itself and still pass durable portable cognitive gain, out-of-context-completeness, excerpt-necessity, and selection-reason tests; Notes attach useful knowledge, context, or non-obvious connections that a thoughtful ordinary reader may not know, notice, or infer on their own, and require non-empty visible `content`. Dense units should still preserve every source span that genuinely passes the gates rather than forcing a top-1/top-2 choice; Highlight and Note anchors may overlap; `source_quote` should be chosen according to the action and remain the smallest complete contiguous local meaning span for that action; the active promptset bundle is `attentional_v2-phase6-v83`
    - subject-continuity / standalone Understanding follow-up remains implemented in Digest prompt `attentional_v2.digest.v23`: prior Understanding in `ReadingMemory` carries narrator / speaker / actor / concept continuity; Digest establishes new subjects, continues known subjects when supported, or explicitly preserves meaningful ambiguity without adding raw prior-source backfill, Ingest reference-resolution fields, or a durable referent store
