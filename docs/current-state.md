...
Last verified: `2026-06-07T09:20:03+08:00`
...
- The `Ingest -> Digest -> Reading Runner settlement` mechanism reframe is implemented; current work is fact alignment, smoke/diagnostic review, and calibration before any formal evaluation promotion.
  ...
  - live cleanup landed:
    ...
    - Ingest prompt `attentional_v2.ingest.v10` calibrates `RecallPriorReading` as retrieval-target prior-reading recall: it describes what earlier understanding runtime should try to find from selected-unit cues, does not require the model to assert that prior memory already exists, treats `CurrentView / Content` as boundary/recall-cue preview rather than already-read memory evidence, looks backward beyond the selected unit rather than summarizing the selected unit itself, avoids paragraph / line / XML identifiers, keeps recall text in the current source text's primary language, preserves source-form names/terms when available, and keeps model-side recall `basis` exactly `selected_source_unit`
    - Ingest tool-loop validation now requires final submitted `memory_recalls[]` to match the `retrieve_unit_memory` action-tool recalls when the tool was called, preventing trace/runtime divergence where the model retrieves on one recall set and finalizes another
