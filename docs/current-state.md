...
- The `Ingest -> Digest -> Reading Runner settlement` mechanism reframe is implemented; current work is fact alignment, smoke/diagnostic review, and calibration before any formal evaluation promotion.
  ...
  - live cleanup landed:
    ...
    - Digest prompt `attentional_v2.digest.v14` now frames `understanding` as concise source-established content from the current source text and frames Marginalia as page-margin reader marks anchored to exact source spans, not generic explanatory annotations; Marginalia may be highlight-only or note-bearing, highlight-only is reserved for intrinsically valuable excerpt-worthy quotes whose value is visible in the quoted words themselves, and each highlight-only item carries a private audit `selection_reason` in `marginalia_audit[]`; the active promptset bundle is `attentional_v2-phase6-v72`
    - subject-continuity / standalone Understanding follow-up remains implemented in Digest prompt `attentional_v2.digest.v14`: prior Understanding in `ReadingMemory` carries narrator / speaker / actor / concept continuity; Digest establishes new subjects, continues known subjects when supported, or explicitly preserves meaningful ambiguity without adding raw prior-source backfill, Ingest reference-resolution fields, or a durable referent store
