...
Last verified: `2026-06-10T20:36:16+08:00`
...
- The `Ingest -> Digest -> Reading Runner settlement` mechanism reframe is implemented; current work is fact alignment, smoke/diagnostic review, and calibration before any formal evaluation promotion.
  ...
  - live cleanup landed:
    ...
    - Runtime now builds a larger paragraph-offset reading lookahead window for Ingest: default `reader_policy.unitize` preview values are `preview_soft_min_chars=3000`, `preview_hard_max_chars=7000`, and `max_lookahead_paragraphs=12`; this is a preview sizing change only and does not change the Ingest output contract
