...
Last verified: `2026-06-21T16:33:19+08:00`
...
- The `Ingest -> Digest -> Reading Runner settlement` mechanism reframe is implemented; current work is fact alignment, smoke/diagnostic review, and calibration before any formal evaluation promotion.
  ...
  - live cleanup landed:
    ...
    - Digest prompt `attentional_v2.digest.v18` now frames `understanding` as concise source-established content from the current source text and frames Marginalia as page-margin reader marks anchored to exact source spans, not generic explanatory annotations; Marginalia may be highlight-only or note-bearing, highlight-only is reserved for quote-only excerpts that remain understandable when lifted out of the book and carry real cognitive, knowledge, aesthetic, emotional, ethical, or expressive value, each highlight-only item carries a private inline `selection_reason` naming both completeness and value, note-bearing content should share what a thoughtful ordinary reader may not notice, know, or infer on their own rather than paraphrasing visible plot/action, and `source_quote` should be the smallest complete contiguous local meaning span; the active promptset bundle is `attentional_v2-phase6-v78`
    - subject-continuity / standalone Understanding follow-up remains implemented in Digest prompt `attentional_v2.digest.v18`: prior Understanding in `ReadingMemory` carries narrator / speaker / actor / concept continuity; Digest establishes new subjects, continues known subjects when supported, or explicitly preserves meaningful ambiguity without adding raw prior-source backfill, Ingest reference-resolution fields, or a durable referent store
