...
Last verified: `2026-06-07T10:41:55+08:00`
...
- The `Ingest -> Digest -> Reading Runner settlement` mechanism reframe is implemented; current work is fact alignment, smoke/diagnostic review, and calibration before any formal evaluation promotion.
  ...
  - live cleanup landed:
    ...
    - Ingest prompt `attentional_v2.ingest.v11` calibrates `RecallPriorReading` as retrieval-target prior-reading recall and adds a model-side boundary closure check: it describes what earlier understanding runtime should try to find from selected-unit cues, does not require the model to assert that prior memory already exists, treats `CurrentView / Content` as boundary/recall-cue preview rather than already-read memory evidence, looks backward beyond the selected unit rather than summarizing the selected unit itself, avoids paragraph / line / XML identifiers, keeps recall text in the current source text's primary language, preserves source-form names/terms when available, keeps model-side recall `basis` exactly `selected_source_unit`, and asks Ingest not to leave terminal punctuation / attached closing marks behind when they belong to the chosen unit
