...
Last verified: `2026-06-08T19:49:32+08:00`
...
- The `Ingest -> Digest -> Reading Runner settlement` mechanism reframe is implemented; current work is fact alignment, smoke/diagnostic review, and calibration before any formal evaluation promotion.
  ...
  - live cleanup landed:
    ...
    - `DEC-115` adds protocol-neutral LLM structured-output transport: Anthropic-compatible profiles keep forced final-output tools, while OpenAI-compatible profiles configured with JSON-object response format can use JSON object plus validator/repair without redefining prompts or tools
    - current `llm_calls.ingest(...)` is the forward-only XML LLM boundary call: exact `end_anchor_text`, boundary-rationale `reason`, and bounded `memory_recalls[]`
    ...
    - current `attentional_v2` LLM calls keep project schemas/tools protocol-neutral; Anthropic-compatible profiles submit structured results through mechanism-private final-output tools such as `submit_ingest_result` and `submit_digest_result`, while OpenAI-compatible JSON-object profiles use JSON object plus validator/repair for Ingest/Digest final output; `retrieve_unit_memory` remains the only live action tool and is left to model `auto` choice
    - runtime next-unit preparation lives outside `Ingest` as `prepare_next_source_unit_for_read`: it prepares source preview/context, calls `Ingest`, performs exact / quote-normalized anchor resolution, trailing-closing-punctuation boundary extension, retry/fallback boundary governance, and hands the accepted source unit to `Digest`
    ...
    - Ingest prompt `attentional_v2.ingest.v13` calibrates `RecallPriorReading` as retrieval-target prior-reading recall and adds model-side boundary governance: it describes what earlier understanding runtime should try to find from selected-unit cues, does not require the model to assert that prior memory already exists, treats `CurrentView / Content` as boundary/recall-cue preview rather than already-read memory evidence, looks backward beyond the selected unit rather than summarizing the selected unit itself, avoids paragraph / line / XML identifiers, keeps recall text in the current source text's primary language, preserves source-form names/terms when available, keeps model-side recall `basis` exactly `selected_source_unit`, asks Ingest not to leave terminal punctuation / attached closing marks behind when they belong to the chosen unit, defines `reason` as the boundary rationale for why the unit starting at the current cursor ends at `end_anchor_text`, and removes the old model-emitted boundary classification field from the live interface
    - Ingest tool-loop validation now requires final submitted `memory_recalls[]` to match the `retrieve_unit_memory` action-tool recalls when the tool was called, preventing trace/runtime divergence where the model retrieves on one recall set and finalizes another; structured-output / LLM failures now propagate as explicit LLM problem codes such as `llm_contract` instead of being normalized into an empty `end_anchor_text` boundary; runtime does not broadly auto-absorb opening punctuation or next-unit markers
