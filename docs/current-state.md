...
Last verified: `2026-09-03T11:06:45+08:00`

## Current Objective
- `TASK-READING-PRODUCT-XIDADUO-FULL-BOOK-LIVE-ACCEPTANCE` is complete. The exact local《悉达多》EPUB at SHA-256 `f239921773ac5abc86527fb78379cbd68cdf2cb901d253e085b2883180984a4f` ran through ordinary `parse_book` / `read_book`, the current `attentional_v2` mechanism, and real `cpa_codex_local` / `gpt-5.6-luna` calls with `reasoning_effort=medium` and `hybrid` memory retrieval. One upstream TLS EOF stopped the fresh attempt after `78` committed Units; normal resume reused reading ID `urn:uuid:f91e6d64-0d70-48e7-a18e-95d7db6504fa`, did not repeat committed work, and reached `completed`. The immutable complete Reading Product has `89` Units over a `16`-chapter / `590`-paragraph BookDocument and `207` Marginalia (`89` Highlights, `118` Notes); `15` chapter compatibility projections were derived from Product Units. The detached minimal Pack exports all `207` items, independently validates and inspects, resolves every quote/position against `22` verified EPUB resources with zero unverifiable hrefs, and returns `unchanged` on both repeat exports. Its producer report has one nonfatal `invalid_publication_identifier` warning because an invalid optional ISBN was omitted; the standalone Pack semantic validator reports zero findings. The local LLM ledger has `325` calls (`324` successful with complete usage, one recovered failure with usage unavailable) and `2,194,331` known tokens; subscription billing leaves actual billed cost unknown. The isolated evidence lives under `reading-companion-backend/state/reading_product_live_acceptance/cpa_luna_xidaduo_full_book_20260903/`. The bounded claim is: **在固定 SHA-256 的真实《悉达多》EPUB 上，当前机制已经通过 CPA Luna 的真实 LLM 调用完成整书阅读，并生成 complete Reading Product、章节兼容投影和独立验证通过的 207-item Annotation Pack。** This is a new run, not migration of historical Xidaduo artifacts, and is not a human quality benchmark or proof of Pages/frontend/API deployment.
...
- `TASK-ANNOTATION-PACK-RECENT-AGENT-ARTIFACT-VALIDATION` is complete with a corrected result and the safe-DOCTYPE follow-through is implemented. The best recent real current-native historical evidence is the July 5 phase9 Xidaduo evaluation segment, not the April phase8 product output. Its `112` records correctly contain `83` bodyless Highlights and `29` body-bearing Notes; all quote/range slices and timestamps round-trip, and the legacy adapter accepts `111`, rejecting only one five-code-point quote that occurs twice in its source unit. The exact EPUB and coherent `16`-chapter/`590`-paragraph source `BookDocument` are traceable through the dataset, and a deterministic renderer replay maps all `112` segment ranges back to exact EPUB hrefs. The exact-resource parser now accepts only one simple HTML5 `<!DOCTYPE html>` in the resource prolog while continuing to reject ENTITY, internal subsets, SYSTEM/PUBLIC, duplicates, malformed XML, and hostile resources. The real Xidaduo source now yields `22` resource texts, all `590` paragraph ranges, and zero unverifiable hrefs. Historical direct export remains out of scope and would still require a persisted coordinate map/co-located EPUB, an explicit export-facing terminal state, and resolution or explicit skipping of the one ambiguous quote. New Reading Product outputs already persist canonical coordinates and a real complete state, so they do not inherit those historical-artifact gaps.
- The Annotation Hub consumer migration remains separately `waiting` under `TASK-ANNOTATION-HUB-V0-MINIMAL-PACK-MIGRATION`, is no longer blocked by Pack implementation, and stays outside this worktree. `DEC-159` explicitly defers GitHub Pages hosting: the reserved schema IRI is not live, and `.github/workflows/annotation-pack-pages.yml` now validates projection bytes only with no upload/deploy permissions or job. This does not block merging the completed core implementation.
...
- The `Ingest -> Digest -> Reading Runner settlement` mechanism reframe is implemented; current work is fact alignment, smoke/diagnostic review, and calibration before any formal evaluation promotion.
  ...
  - latest full-window diagnostic:
    ...
    - evidence caveat: post-run manual review found that Huochu unit `13` in the parent run swallowed a Digest `network_blocked` failure into an empty `digest_complete` row, so Huochu unit `13` and downstream Huochu continuity in this packet are diagnostic / tainted until a Huochu-only full-window rerun is produced after `DEC-152`.
  ...
  - live cleanup landed:
    ...
    - `DEC-120` switches live Ingest preview capacity from the interim `7000`-character hard budget to token-bounded paragraph-aligned assembly, and bumps Ingest to v16 so only the committed first unit receives a boundary rationale while later preview partitions stay compact audit metadata
    ...
    - `DEC-129` simplifies the live Digest Marginalia item contract: Digest prompt `attentional_v2.digest.v11` / XML spec v11 / promptset `attentional_v2-phase6-v69` / output contract `digest_understanding_response_marginalia_json_v5` asks for only `source_quote` plus optional `content`; empty, null, or omitted `content` is highlight-only, non-empty `content` is note-bearing, and legacy `prior_link` / `outside_link` / `search_intent` fields remain compatibility-read metadata rather than live model-facing output fields
    - `DEC-130` tightens the live Digest highlight-only Marginalia boundary: Digest prompt `attentional_v2.digest.v12` / XML spec v12 / promptset `attentional_v2-phase6-v70` keeps output contract `digest_understanding_response_marginalia_json_v5`, but now teaches highlight-only as excerpt-worthy quotes that can stand alone, while structurally important but context-dependent spans should be skipped or become note-bearing Marginalia
    - `DEC-131` adds the intrinsic excerpt-value gate to highlight-only Marginalia: Digest prompt `attentional_v2.digest.v13` / XML spec v13 / promptset `attentional_v2-phase6-v71` keeps output contract `digest_understanding_response_marginalia_json_v5`, but now says self-contained is necessary but not sufficient; quote-only highlights must also carry value through the original wording, image, insight, emotional force, conceptual compression, or compact principle
    - `DEC-132` adds a private highlight-selection audit reason to live Digest Marginalia: Digest prompt `attentional_v2.digest.v14` / XML spec v14 / promptset `attentional_v2-phase6-v72` / output contract `digest_understanding_response_marginalia_json_v6` requires one `marginalia_audit[]` item with `selection_reason` for each highlight-only Marginalia quote; note-bearing Marginalia keeps its reason in visible `content`
    - `DEC-133` moves the private highlight-selection reason inline: Digest prompt `attentional_v2.digest.v15` / XML spec v15 / promptset `attentional_v2-phase6-v73` / output contract `digest_understanding_response_marginalia_json_v7` requires highlight-only Marginalia to carry `marginalia[].selection_reason`, while new live outputs no longer emit top-level `marginalia_audit[]`
    - `DEC-134` tightens live Digest quote-span selection: Digest prompt `attentional_v2.digest.v16` / XML spec v16 / promptset `attentional_v2-phase6-v74` keeps output contract `digest_understanding_response_marginalia_json_v7`, but now requires Marginalia `source_quote` to be the smallest complete contiguous local meaning span rather than a clipped phrase, isolated term, or partial image
    - `DEC-135` tightened the then-live Ingest first-unit boundary selection: Ingest prompt `attentional_v2.ingest.v18` / XML spec v18 / promptset `attentional_v2-phase6-v75` kept the same boundary contract but added a same-local-function rule so adjacent setup, character construction, scene build, argument support, example chains, or emotional turns are not split merely because their smaller facets can be titled separately; `DEC-136` carries that principle into the current prefix-grouped v19 contract
    - `DEC-136` changes live Ingest from first-partition equality to prefix-grouped unit selection: Ingest prompt `attentional_v2.ingest.v19` / XML spec v19 / promptset `attentional_v2-phase6-v76` / output contract `ingest_unit_boundary_preview_partition_json_v4` keeps `preview_partition[]` as the fine-grained audit map while allowing the committed Digest unit to cover `preview_partition[0..k]`, with unit-size guidance `300 / 900 / 1600` source tokens
    - `DEC-137` tightens live Digest highlight-only Marginalia selection: Digest prompt `attentional_v2.digest.v17` / XML spec v17 / promptset `attentional_v2-phase6-v77` keeps output contract `digest_understanding_response_marginalia_json_v7`, but now requires quote-only highlights to pass both an out-of-context completeness gate and a value gate, with `selection_reason` naming both
    ...
    - `DEC-142` makes live Digest Marginalia density-aware for high-value units: Digest prompt `attentional_v2.digest.v20` / XML spec v20 / promptset `attentional_v2-phase6-v80` keeps output contract `digest_understanding_response_marginalia_json_v7`, but no longer frames Marginalia as a top-1/top-2 selection task; dense units should preserve all genuinely durable thought nodes, merge adjacent sentences when they form one complete reusable idea, and treat "smallest complete" source quotes as the shortest contiguous span that preserves the full reusable idea rather than the shortest possible sentence
    - `DEC-143` tightens live Digest highlight-only Marginalia selection again: Digest prompt `attentional_v2.digest.v21` / XML spec v21 / promptset `attentional_v2-phase6-v81` keeps output contract `digest_understanding_response_marginalia_json_v7`, but requires highlight-only quotes to pass durable-value, out-of-context-completeness, and excerpt-necessity gates; private `selection_reason` must explain why the quote is more than local evidence, scene importance, or a strong fact from the current book, while plain reality-facing claims remain eligible when they offer reusable cognitive value
    - `DEC-144` adds fixed high-throughput LLM scheduling for trusted local targets: `concurrency_strategy="fixed"` lets OpenCode Go diagnostics use the declared `24`-way capacity without adaptive local gate backoff, while quota/auth cooldown behavior remains intact
    - `DEC-145` adds layered transient-LLM recovery for Digest Marginalia diagnostics: gateway retries still handle provider attempts, the smoke runner may retry the same unit once from the same cursor with bounded timeout escalation, and transient per-segment exhaustion becomes a `partial` diagnostic rather than a whole-run failure by default
    - `DEC-146` tightens highlight-only Marginalia again around durable portable cognitive gain: Digest prompt `attentional_v2.digest.v22` / XML spec v22 / promptset `attentional_v2-phase6-v82` keeps output contract `digest_understanding_response_marginalia_json_v7`, but now says cruelty, suffering, historical importance, emotional force, moral shock, or other strong facts are not enough unless the quote itself crystallizes a reusable insight, distinction, mechanism, warning, model, or self-correction; private `selection_reason` must name that specific gain and fail if it would fit many similar passages by swapping names or situations
    - `DEC-147` makes live Digest Marginalia parallel rather than mutually exclusive: Digest prompt `attentional_v2.digest.v23` / XML spec v23 / promptset `attentional_v2-phase6-v83` / output contract `digest_understanding_response_marginalia_json_v8` now says Marginalia include Highlights and Notes; `marginalia[].kind` is required for new live output, Highlights preserve portable source text with private `selection_reason`, Notes attach reader-facing cognitive surplus through non-empty `content`, and overlapping Highlight / Note anchors are allowed rather than deduped by quote
    ...
    - Ingest prompt `attentional_v2.ingest.v19` / promptset `attentional_v2-phase6-v76` keeps the reviewed window-partition selector plus preview-partition audit map, tightens recall ownership, and adds prefix-grouped Digest-unit selection: final output no longer includes `memory_recalls[]`; `preview_partition[]` remains a fine-grained semantic map, while the committed `unit` may cover a continuous left prefix `preview_partition[0..k]` and must end at one complete partition boundary
    - Runtime derives `unit_partition_range`, `unit_partition_titles`, `unit_estimated_token_count`, `unit_size_policy`, and `unit_size_status` as mechanism-private audit metadata; later `preview_partition[]` entries beyond the committed prefix are future-source audit/planning metadata and do not affect Digest input, Unit Memory retrieval, Recent Reading Memory, or public API payloads
    - Ingest tool-loop validation now treats `retrieve_unit_memory` action-tool args as the only recall authority: non-empty tool calls enforce visible unit boundary, same-language recall text, and `basis == selected_source_unit`; invalid recall-tool args are recorded as non-fatal `invalid_tool_noop` / `invalid_skipped` retrieval degradation and the reading cycle continues without Unit Memory retrieval; empty optional recall tool calls are treated as no-op / not-requested events; final output validation ignores legacy final `memory_recalls[]` echoes and only checks boundary / preview-partition correctness. Structured-output / LLM failures propagate as explicit LLM problem codes such as `llm_contract` instead of being normalized into an empty boundary; runtime does not broadly auto-absorb opening punctuation or next-unit markers
    ...
    - Digest LLM / contract failures now preserve cursor safety: the Runner no longer manufactures an empty Digest result after `ReaderLLMError`, and settlement / memory writeback / cursor advance happen only after a valid content-bearing `understanding` and `response`
    - Digest prompt `attentional_v2.digest.v24` now frames `understanding` as concise source-established content from the current source text and frames Marginalia as page-margin reader marks anchored to exact source spans, not generic explanatory annotations; Marginalia include Highlights and Notes as two independent reader actions rather than a content-empty/content-bearing choice. Highlights preserve exact source text worth carrying forward by itself and must pass intrinsic quote-value, quote-itself, out-of-context-completeness, excerpt-necessity, and selection-reason-audit gates: `selection_reason` may only identify value already visible in the quote, not rescue a local scene/fact/testimony by abstracting it afterward. Notes attach useful knowledge, context, or non-obvious connections that a thoughtful ordinary reader may not know, notice, or infer on their own, and require non-empty visible `content`. Dense units should still preserve every source span that genuinely passes the gates rather than forcing a top-1/top-2 choice; Highlight and Note anchors may overlap; `source_quote` should be chosen according to the action and remain the smallest complete contiguous local meaning span for that action; the active promptset bundle is `attentional_v2-phase6-v84`
    - subject-continuity / standalone Understanding follow-up remains implemented in Digest prompt `attentional_v2.digest.v24`: prior Understanding in `ReadingMemory` carries narrator / speaker / actor / concept continuity; Digest establishes new subjects, continues known subjects when supported, or explicitly preserves meaningful ambiguity without adding raw prior-source backfill, Ingest reference-resolution fields, or a durable referent store
  ...
  - stop declaration:
    - do not continue implementing `C设计10` consolidation, the old concrete-node XML context migration, `C设计12` prompt assembly migration, ActiveTension expansion, Thread / Progression / Development memory, or broader Eval-1 follow-up as the next mechanism direction
  ...
  - Digest v16 Marginalia five-book diagnostic:
    ...
    - outcome: `100` runner units across `5` segments, summary status `pass_with_caveats`, `123` Marginalia items (`99` highlight-only, `24` note-bearing), `13` `quote_too_broad` caveats, and `0` hard failures
    - report: `reading-companion-backend/eval/runs/attentional_v2/digest_marginalia_v16_5book_parallel_20units_20260621/analysis/digest_marginalia_v16_5book_parallel_20units/marginalia_smoke_report.md`
  ...
  - Digest v21 Marginalia five-book diagnostic:
    ...
    - recovery follow-up: `DEC-145` made future Digest Marginalia focused diagnostics default to `--failure-policy partial`; `DEC-150` extends that recovery posture for long-running diagnostics so provider-side transient instability waits and retries the same cursor under a longer budget (`6` extra unit retries, default delay schedule `0,120,300,600,900,1200`, and `3600s` per-unit transient recovery budget) before preserving a segment as `partial`
  ...
  - Digest v23 Marginalia five-book diagnostic:
    ...
    - recovery follow-up: `DEC-148` landed private provider-connection diagnostics plus delayed same-cursor unit recovery; the retry below completed with useful v23 evidence before `DEC-149` promoted the next live prompt
  ...
  - Digest v23 Marginalia five-book diagnostic retry with provider recovery:
    ...
    - outcome: summary status `pass_with_caveats`; `88` runner units across 5 segments, `170` Marginalia items (`133` Highlights, `37` Notes), `20` `quote_too_broad` caveats, `1` `possibly_generic` caveat, zero hard/partial failures, `21` unit recovery attempts, and `9` recovered units
    - report: `reading-companion-backend/eval/runs/attentional_v2/digest_marginalia_v23_5book_parallel_20units_recovery_retry1_20260702/analysis/digest_marginalia_v23_5book_parallel_20units_recovery_retry1/marginalia_smoke_report.md`
  - Digest v24 Marginalia five-book diagnostic:
    - run id: `digest_marginalia_v24_5book_parallel_20units_20260703`
    - job id: `bgjob_digest_marginalia_v24_5book_parallel_20units_20260703`
    - status: completed with summary status `partial`; run ledger status is `partial`
    - scope: active unique-note source-normalized v1.2 five-segment dataset, `segment_workers=5`, up to `20` accepted units per segment, `failure_policy=partial`
    - purpose: test the live intrinsic quote-value Highlight gate in `attentional_v2.digest.v24` / promptset `attentional_v2-phase6-v84` against the same five-segment diagnostic shape used for v22/v23 review
  ...
  - retrieval repair status:
    ...
    - Phase 6 recall-specificity prompt calibration:
      ...
      - expected effect: recall intentions should be focused, retrieval-friendly prior-understanding descriptions; they should avoid paragraph / line / XML identifiers, avoid noun-list splitting, prefer one strong recall over several weak recalls, and retrieve relevant continuity even when the current unit is intelligible on its own
    ...
    - post-selection-cap `text_only` diagnostic smoke:
      ...
      - segment: `xidaduo_private_zh__segment_1`
      - result: intentionally stopped after selection-cap evidence was collected; no summary aggregate/report/usage files were generated
      - report: `docs/implementation/new-reading-mechanism/codex/reports/UnitMemory-Retrieval-TextOnly-PostSelectionCap-Smoke-Report v0.md`
    ...
    - current recall-language contract repair:
      ...
      - code behavior after `DEC-127`: `retrieve_unit_memory` action-tool preflight receives the current source text and rejects recall args that clearly do not use the current source text's primary language; `submit_ingest_result` no longer accepts model-authored `memory_recalls[]` as a final-output field, so final recall echoes are ignored rather than matched against the tool call
      - pre-contract smoke:
  ...
  - next step:
    ...
    - use `docs/implementation/new-reading-mechanism/source-normalization-design.md`, `docs/implementation/new-reading-mechanism/mechanism-pattern-ledger.md` entry 19, `DEC-116`, `DEC-117`, `DEC-118`, `DEC-120`, `DEC-121`, `DEC-122`, `DEC-123`, `DEC-124`, `DEC-125`, `DEC-127`, `DEC-135`, `DEC-136`, `DEC-137`, `DEC-138`, `DEC-139`, `DEC-140`, `DEC-142`, `DEC-143`, `DEC-146`, `DEC-147`, `DEC-148`, `DEC-149`, and `DEC-150` for the accepted design explanation of why window-partition Ingest improves next-unit selection, why live Ingest records `preview_partition[]` titles as mechanism-private audit metadata, why runtime first moved away from paragraph-count previews, why live preview capacity is token-bounded, why the current v16 token window is larger than the initial live v16 value, why footnotes/noise should be handled by upstream source normalization rather than Ingest skip behavior, how Source Normalization applies only to newly parsed books, why v1.1 preserves markup context while protecting literary body text, why v1.2 no longer uses live whole-book LLM source-flow classification, why Unit Memory recall intent now has only one model-authored surface, why Ingest should not split one setup / portrait / scene / support chain merely because smaller facets can be titled, why highlight-only Marginalia must remain understandable and valuable when lifted out of book context, why note-bearing Marginalia should provide reader-visible cognitive increment rather than shallow paraphrase, why the live prompt now demotes ordinary close-reading technique commentary in favor of useful knowledge / non-obvious reader surplus, why invalid Ingest recall-tool args should skip Unit Memory retrieval rather than blocking source-unit progress, why dense high-value units should not be forced into top-1/top-2 selection, why highlight-only quotes now need excerpt necessity rather than merely local book force, why Highlights and Notes are parallel actions with explicit `marginalia[].kind` rather than a single content-derived choice, why provider connection details stay private while focused diagnostics use delayed same-cursor recovery, why Highlight selection must audit intrinsic quote value rather than using `selection_reason` to abstract local evidence after the fact, and why long-running diagnostics should wait through bounded provider-side transient instability before stopping a segment
...
- Historical concrete-node XML prompt / Recent Reading Memory full active diagnostic machine run has completed; post-run report is preserved for reference.
  - purpose:
    ...
    - check whether basic concrete-node reading behavior remained healthy
  ...
  - mode:
    ...
    - historical XML toggle was used for that run; the current live Digest path is always XML and does not use a legacy prompt-assembly toggle
  ...
  - next step:
    ...
    - do not keep iterating on the old concrete-node XML context assembly or Recent Memory consolidation as the next implementation path unless the new ingest/digest feasibility audit explicitly re-adopts a slice
...
- `reading_impression` overlaps with the new `recent_reading_memory` layer and should be cleaned up during a future reaction/Digest-contract tuning pass.
  - current fact:
    - `reading_impression` predates `recent_reading_memory`; it was introduced as the temporary read-after impression for the naturalized Digest contract
    - now that `recent_reading_memory` owns near-term per-unit semantic memory, `reading_impression` can duplicate that role and can nudge reports / prompts toward essay-like analysis
  - current boundary:
...
- Recent Reading Memory micro diagnostics completed and are pending review.
  ...
  - status facts:
    ...
    - beginning-of-book `p1-p24` diagnostic completed with exit code `0`; strict LLM health passed with `22` traces, `22` successes, `0` errors, and `0` fallback-backed evidence
    - beginning-of-book `p1-p24` covered `10` read units and produced `10` active `recent_reading_memory` entries; unit 1 produced no Recent Memory despite containing the opening frame / core question
    ...
    - historical retry diagnostics for the old concrete reading node are preserved as diagnostic history; they are superseded by the current Ingest / Digest naming and live XML Digest path
...
- Memory / Planning / Minimal Eval implementation track for optimizing the existing `attentional_v2` mechanism is closed after Slice 8H.
  - status:
    ...
    - Slice 5A's compact detour lifecycle metadata and compact `read_audit` navigation/detour evidence are historical after `DEC-105` and no longer current runtime/audit surfaces
    ...
    - Slice 7A minimal eval asset inventory and evidence wiring is implemented, and `Slice7A-Minimal-Eval-Asset-Inventory-and-Evidence-Wiring-Post-implementation-Report v0.md` is accepted
    ...
    - `Slice7B-Minimal-Eval-Smoke-Harness-and-Evidence-Availability-Validation-Pre-implementation-Brief v0.md` is accepted
    - Slice 7B minimal eval smoke harness and evidence availability validation is implemented, and `Slice7B-Minimal-Eval-Smoke-Harness-and-Evidence-Availability-Validation-Post-implementation-Report v0.md` is accepted
    - Slice 7B added a stdlib-only smoke validator for the Slice 7A manifest and subprocess tests; no eval, judge calls, benchmark jobs, reading jobs, eval runner changes, or runtime mechanism changes were introduced
    - `Slice8A-Post-implementation-Review-and-Minimal-Eval-Readiness-Gate-Pre-implementation-Brief v0.md` is accepted
    - Slice 8A doc-only readiness gate is landed, and `Slice8A-Post-implementation-Review-and-Minimal-Eval-Readiness-Gate-Post-implementation-Report v0.md` is accepted
    - Slice 8A records that no prerequisite runtime patch or eval-runner patch is currently indicated before a later Minimal Eval Suite run brief
    ...
    - `Slice8G-Minimal-Eval-Suite-Smoke-Closure-and-Evidence-Interpretation-Report v0.md` is accepted
  ...
  - current posture:
    ...
    - landed Phase-1 implementation:
      ...
      - `Memory Quality`:
        ...
        - standard runtime audit now links Digest memory uptake to deterministic Digest -> Reading Runner settlement transaction deltas through `read_audit.jsonl` and `settlement_audit.jsonl`
        - current `attentional_v2` mainline cursor semantics have now shifted from sentence-id traversal to paragraph-offset source spans:
          ...
          - `Ingest` sees an adaptive paragraph-offset preview and returns `unit.end_paragraph_n` plus `unit.end_at`
          - `Reading Runner` resolves that boundary into an accepted unit span, advances the source cursor, and appends `_mechanisms/attentional_v2/runtime/unit_span_ledger.jsonl`
...
- A new post-Phase-9 implementation line is now opening for `attentional_v2`:
  ...
  - `Phase B` is now also landed:
    ...
    - that branch allowed bounded supplemental context through `active recall` or `look-back`; those helpers are retired after `DEC-105`
  ...
  - `Phase C.1` is now landed as the first packetization seam:
    ...
    - the then-current concrete reading node received a `state_packet.v1` context packet that explicitly separated:
  ...
  - `Phase F1` is now landed as the concrete reading-node contract and prompt-packaging cutover:
  ...
  - the unit-internal anchor-selection repair is now also landed on that baseline:
    ...
    - if a single sentence would lose its meaning in isolation, `Digest` may instead use the smallest multi-sentence span that stays self-sufficient
  ...
  - the April 12 post-Phase-D smoke has now finished and is interpreted as follows:
    ...
    - the current diagnosis is:
      - the slowest old concrete reading-node and unitization calls were not driven by abnormally large local正文 spans
...
- Preserve the corrected real-artifact audit: current phase9 kind/body and read-surface anchors pass, and the safe-DOCTYPE parser repair is complete. Historical direct export remains intentionally unimplemented and would require persisted eval segment-to-source coordinates, a co-located EPUB, export-facing runtime finalization, and explicit resolution or skipping of one ambiguous quote. Keep the April phase8 output only as historical migration evidence; do not retrofit old eval artifacts merely to prove the new Product path.
...
## Active Risks
- The reserved GitHub Pages-shaped mapping remains buildable and byte-checkable, but `DEC-159` explicitly defers public hosting. The validation-only workflow must not upload or deploy on `main`; any future activation requires a new explicit decision and served-byte comparison.
