...
Last verified: `2026-06-21T14:14:30+08:00`
...
- The `Ingest -> Digest -> Reading Runner settlement` mechanism reframe is implemented; current work is fact alignment, smoke/diagnostic review, and calibration before any formal evaluation promotion.
  ...
  - live cleanup landed:
    ...
    - Digest prompt `attentional_v2.digest.v16` now frames `understanding` as concise source-established content from the current source text and frames Marginalia as page-margin reader marks anchored to exact source spans, not generic explanatory annotations; Marginalia may be highlight-only or note-bearing, highlight-only is reserved for intrinsically valuable excerpt-worthy quotes whose value is visible in the quoted words themselves, each highlight-only item carries a private inline `selection_reason`, and `source_quote` should be the smallest complete contiguous local meaning span; the active promptset bundle is `attentional_v2-phase6-v74`
