...
Last updated: `2026-06-10T20:36:16+08:00`
...
- Next: Five-window Long Span vNext judged diagnostic completed for the current Ingest -> Digest -> Reading Runner settlement mechanism and is review_pending in the run ledger. Current live Ingest returns end_anchor_text/reason plus bounded memory_recalls[]; retrieve_unit_memory triggers runtime Unit Memory retrieval/selection before Digest, and Digest receives top-level ReadingMemory assembled from hot current-chapter Understanding plus selected long-distance Unit Memory Understanding. Unit Memory retrieval repair under docs/implementation/new-reading-mechanism/unit-memory-retrieval-repair-validation-plan.md now has diagnostic proof for both text_only and live hybrid mechanics. Text_only smokes prove prompt-visible retrieved Understanding; R14/R15 boundary fixes and R16 auxiliary-backed Understanding gating are validated. The earlier post-R16 selected_but_not_rendered_count=3 reading is now classified as stopped-smoke pending selection, not a live rendering failure. Local hybrid readiness passes with sqlite-vec plus Ollama App `qwen3-embedding:0.6b`; after an initial embedding-timeout degradation, the raised-timeout live hybrid smoke `attentional_v2_unit_memory_hybrid_smoke_xidaduo_phase2_r1_20260606` reached health status ok with 47 unit_understanding vector rows, 2 real Qwen query cache rows, 21 dense docs, 21 lexical docs, 11 selected units, 11 retrieved ReadingMemory lines, and 8 unique rendered retrieved units. Runtime preview for Ingest now uses a larger reading lookahead window with `preview_soft_min_chars=3000`, `preview_hard_max_chars=7000`, and `max_lookahead_paragraphs=12`, without changing the Ingest output contract. Ingest prompt `attentional_v2.ingest.v13` / promptset `attentional_v2-phase6-v63` now calibrates recall text as retrieval-target prior-reading memory, treats CurrentView as cue preview rather than prior-memory evidence, avoids asserting uncertain prior facts, forbids paragraph / line / XML identifiers, adds a boundary closure check so selected anchors include punctuation / closing marks that complete the chosen unit without absorbing next-unit opening markers, defines `reason` as the boundary rationale for why the unit starting at the current cursor ends at `end_anchor_text` rather than as a separate source-span selection, and removes the old model-emitted boundary classification field from the live interface. Ingest structured-output / LLM failures now propagate as explicit LLM problem codes such as `llm_contract` instead of being normalized into an empty `end_anchor_text` and settled as fallback units. Next action: review retrieved-memory usefulness/relevance or decide whether to launch a separately authorized formal evaluation; do not claim product quality or update evidence catalog from these diagnostic smokes.
- Jobs:
