...
Last updated: `2026-06-13T10:39:34+08:00`
...
- Next: Five-window Long Span vNext judged diagnostic completed for the current Ingest -> Digest -> Reading Runner settlement mechanism and is review_pending in the run ledger. Current live Ingest returns `unit.end_paragraph_n` / `unit.end_at`, optional `reason`, and bounded `memory_recalls[]`; `retrieve_unit_memory` triggers runtime Unit Memory retrieval/selection before Digest, and Digest receives top-level ReadingMemory assembled from hot current-chapter Understanding plus selected long-distance Unit Memory Understanding. Unit Memory retrieval repair under docs/implementation/new-reading-mechanism/unit-memory-retrieval-repair-validation-plan.md now has diagnostic proof for both text_only and live hybrid mechanics. Text_only smokes prove prompt-visible retrieved Understanding; R14/R15 boundary fixes and R16 auxiliary-backed Understanding gating are validated. The earlier post-R16 selected_but_not_rendered_count=3 reading is now classified as stopped-smoke pending selection, not a live rendering failure. Local hybrid readiness passes with sqlite-vec plus Ollama App `qwen3-embedding:0.6b`; after an initial embedding-timeout degradation, the raised-timeout live hybrid smoke `attentional_v2_unit_memory_hybrid_smoke_xidaduo_phase2_r1_20260606` reached health status ok with 47 unit_understanding vector rows, 2 real Qwen query cache rows, 21 dense docs, 21 lexical docs, 11 selected units, 11 retrieved ReadingMemory lines, and 8 unique rendered retrieved units. Runtime preview for Ingest uses a larger reading lookahead window with `preview_soft_min_chars=3000`, `preview_hard_max_chars=7000`, and `max_lookahead_paragraphs=12`; prompt `attentional_v2.ingest.v14` / promptset `attentional_v2-phase6-v64` now promotes the reviewed window-partition selector, committing only the first coherent reading unit from that lookahead and treating paragraph edges as cues rather than defaults. `DEC-116` records the accepted interpretation: Ingest is a bounded-lookahead planner that uses later preview text as boundary evidence while leaving Digest focused on the first accepted unit; full `preview_partition[]` titles remain a future candidate rather than the current live contract. The working design for that candidate now lives at docs/implementation/new-reading-mechanism/ingest-next-unit-optimization-design.md. Ingest structured-output / LLM failures now propagate as explicit LLM problem codes such as `llm_contract` instead of being normalized into an empty boundary and settled as fallback units. Next action: review retrieved-memory usefulness/relevance or decide whether to launch a separately authorized formal evaluation; if prompt evolution resumes, use the optimization design doc and test `preview_partition[]` titles separately rather than editing the v14 baseline in place.
- Jobs:
