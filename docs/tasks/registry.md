...
Last updated: `2026-06-06T15:18:40+08:00`
...
- Next: five-window Long Span vNext judged diagnostic completed for the current `Ingest -> Digest -> Reading Runner settlement` mechanism and is `review_pending` in the run ledger. Current live `Ingest` returns boundary fields plus bounded `memory_recalls[]`; when recalls are present, the `retrieve_unit_memory` action tool lets Reading Runner execute Unit Memory retrieval/selection before calling `Digest`. `Digest` receives top-level `ReadingMemory` assembled from hot current-chapter Understanding plus selected long-distance Unit Memory Understanding, with `5K` hot, `10K` retrieved, and `15K` total prompt-facing estimated-token budgets. Current `attentional_v2` structured LLM outputs are submitted through forced final-output tools; output-contract failures use public problem code `llm_contract`. Unit Memory is the current content-neutral long-distance memory substrate under `DEC-110`; retired Detour/source-backread/source-skill, concept/thread structured-memory interfaces, and content-type classifiers are historical only. Diagnostic review found that Unit Memory retrieval is structurally present but not mechanism-effective yet: Digest `ReadingMemory` rendered hot memory, but retrieved long-distance memory had `retrieved_line_count=0` across all `531` Digest calls, hybrid degraded before dense retrieval was validated, and text-only fallback produced almost no candidates. The staged repair/validation track in `docs/implementation/new-reading-mechanism/unit-memory-retrieval-repair-validation-plan.md` is now in progress: Phase 0 health packet reproduced the five-window failure; Phase 1/5 trace-rendering repair has begun; Phase 2 sqlite-vec loading is repaired but dense retrieval remains blocked by missing/unreachable local Ollama; Phase 3 text-only FTS repair has a known-answer Chinese recall test; Phase 4 boundary-governance repair has begun. Next finish deterministic selected/rendered checks, broaden text-only probes, make horizon gates explainable, prove selected non-empty Understanding renders into Digest `ReadingMemory`, then run a no-judge smoke. Do not update evidence catalog or claim product quality until prompt-visible retrieved Understanding memory is proven.
- Jobs:
