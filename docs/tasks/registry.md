...
Last updated: `2026-06-06T15:56:51+08:00`
...
- Next: five-window Long Span vNext judged diagnostic completed for the current `Ingest -> Digest -> Reading Runner settlement` mechanism and is `review_pending` in the run ledger. Current live `Ingest` returns boundary fields plus bounded `memory_recalls[]`; when recalls are present, the `retrieve_unit_memory` action tool lets Reading Runner execute Unit Memory retrieval/selection before calling `Digest`. `Digest` receives top-level `ReadingMemory` assembled from hot current-chapter Understanding plus selected long-distance Unit Memory Understanding, with `5K` hot, `10K` retrieved, and `15K` total prompt-facing estimated-token budgets. Unit Memory is the current content-neutral long-distance memory substrate under `DEC-110`; retired Detour/source-backread/source-skill, concept/thread structured-memory interfaces, and content-type classifiers are historical only. Diagnostic review found that Unit Memory retrieval is structurally present but not mechanism-effective yet: Digest `ReadingMemory` rendered hot memory, but retrieved long-distance memory had `retrieved_line_count=0` across all `531` Digest calls, hybrid degraded before dense retrieval was validated, and text-only fallback produced almost no candidates. The staged repair/validation track in `docs/implementation/new-reading-mechanism/unit-memory-retrieval-repair-validation-plan.md` is now in progress: Phase 0 health packet reproduced the five-window failure; Phase 1/5 deterministic trace-rendering repair now passes; Phase 2 sqlite-vec loading is repaired but dense retrieval remains blocked by missing/unreachable local Ollama; Phase 3 deterministic text-only repair now covers Chinese recall-meta wording, English concept recall, and multi-recall aggregation; Phase 4 deterministic boundary/horizon repair now records numeric gate counts and retries after accepted boundary fallback. A first `text_only` smoke (`attentional_v2_unit_memory_text_only_smoke_value_20260606`) completed the read loop but failed the wrapper's strict LLM-health gate; its health packet exposed that the Runner was excluding the whole active Recent Reading Memory store from retrieval, effectively removing all prior units. That exclusion bug is now fixed in code but not yet re-smoked. Next rerun a no-judge post-repair `text_only` smoke to prove prompt-visible retrieved Understanding memory. Do not update evidence catalog or claim product quality until prompt-visible retrieved Understanding memory is proven.
- Jobs:
