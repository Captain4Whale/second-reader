...
Last updated: `2026-06-06T16:38:37+08:00`
...
- Next: five-window Long Span vNext judged diagnostic completed for the current `Ingest -> Digest -> Reading Runner settlement` mechanism and is `review_pending` in the run ledger. Current live `Ingest` returns boundary fields plus bounded `memory_recalls[]`; when recalls are present, the `retrieve_unit_memory` action tool lets Reading Runner execute Unit Memory retrieval/selection before calling `Digest`. `Digest` receives top-level `ReadingMemory` assembled from hot current-chapter Understanding plus selected long-distance Unit Memory Understanding, with `5K` hot, `10K` retrieved, and `15K` total prompt-facing estimated-token budgets. Unit Memory is the current content-neutral long-distance memory substrate under `DEC-110`; retired Detour/source-backread/source-skill, concept/thread structured-memory interfaces, and content-type classifiers are historical only. Diagnostic review found that Unit Memory retrieval was structurally present but initially not mechanism-effective: Digest `ReadingMemory` rendered hot memory, but retrieved long-distance memory had `retrieved_line_count=0` across all `531` Digest calls. The staged repair/validation track in `docs/implementation/new-reading-mechanism/unit-memory-retrieval-repair-validation-plan.md` is now in progress: deterministic phases 1/3/4/5 pass; the first `text_only` smoke (`attentional_v2_unit_memory_text_only_smoke_value_20260606`) exposed that Runner was excluding the whole active Recent Reading Memory store from retrieval; that R9 exclusion bug is fixed. Post-R9 `text_only` diagnostic smoke (`attentional_v2_unit_memory_text_only_smoke_xidaduo_post_r9_20260606`) was intentionally stopped after retrieval-success evidence was collected; its health packet is `ok` with `57` Unit Memory entries, `353` retrieval docs, `selected_unit_count=71`, `renderable_selected_unit_count=29`, and `retrieved_line_total=45`. Post-R9 relevance review found useful continuity recall but also auxiliary-surface pollution from terminology / note-cluster units; R10 now prioritizes `unit_understanding` lexical weight over source / annotation / response, and R11 records exact rendered retrieved unit ids in selection traces. Next run a small no-judge post-R10/R11 `text_only` smoke to confirm retrieved lines still appear and relevance improves. Hybrid dense retrieval remains blocked by missing/unreachable local Ollama / Qwen embedding service, so do not claim hybrid success or update evidence catalog.
- Jobs:
