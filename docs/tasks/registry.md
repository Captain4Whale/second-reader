...
Last updated: `2026-06-21T15:25:22+08:00`
...
- Next: Five-window Long Span vNext judged diagnostic remains review_pending in the run ledger, and Unit Memory retrieval repair now has diagnostic proof for both text_only and live hybrid mechanics. Current live Ingest prompt `attentional_v2.ingest.v19` / promptset `attentional_v2-phase6-v76` keeps the reviewed token-bounded preview and whole-window `preview_partition[]` audit map, but changes the committed Digest-unit rule: the model first maps fine-grained semantic partitions, then selects a continuous left prefix `preview_partition[0..k]` as `unit`, ending only at a complete partition boundary. The final Ingest output still exposes only `unit.end_paragraph_n` / `unit.end_at`, `preview_partition[]`, and optional committed-unit `reason`; final `memory_recalls[]` remains retired. Runtime derives private/audit `unit_partition_range`, `unit_partition_titles`, `unit_estimated_token_count`, `unit_size_policy`, and `unit_size_status`; `preview_partition_audit[]` still resolves fine-grained later boundaries without moving the cursor. Digest-unit size guidance is now `unit_soft_min_tokens=300`, `unit_target_max_tokens=900`, and `unit_hard_max_tokens=1600`: length is an audit/selection guardrail, not a runtime rejection rule, and merging is allowed only when adjacent fine partitions remain part of a larger local semantic movement. Runtime builds the visible Ingest preview with unchanged token-bounded, paragraph-aligned assembly: `preview_soft_min_tokens=1600`, `preview_target_max_tokens=3000`, `preview_hard_max_tokens=4200`, `emergency_max_preview_paragraphs=200`, and old char-budget / paragraph-count snapshots ignored as normal stopping rules. Unit Memory recall intent is expressed only through `retrieve_unit_memory` action-tool args; runtime derives private/audit `memory_recalls[]` from non-empty tool args, treats empty optional action-tool calls as no-op / not-requested events, and ignores legacy final-output recall echoes. `retrieve_unit_memory` keeps its recall schema, validates same-language / `selected_source_unit` basis for non-empty recall targets during tool preflight, treats empty calls as no-op, and does not require `preview_partition[]`. Digest is live as `attentional_v2.digest.v16` / promptset `attentional_v2-phase6-v74` inside active bundle `attentional_v2-phase6-v76`: canonical visible-note output is `marginalia[]`, each item requires `source_quote` and accepts optional/empty `content` for highlight-only marks; highlight-only Marginalia is reserved for intrinsically valuable excerpt-worthy quotes that can stand alone, carries one private inline `selection_reason`, and note-bearing Marginalia keeps its reason in visible `content`. `annotations[]`, `surfaced_reactions`, legacy link/search metadata, and public `reaction_*` fields remain compatibility aliases. Key decisions: `DEC-116` records why bounded lookahead improves first-unit selection; `DEC-117` records the preview-partition audit contract; `DEC-120`/`DEC-121` record token-bounded preview calibration; `DEC-127` records recall-authority cleanup; `DEC-128` through `DEC-135` record Marginalia and same-local-function prompt refinements; `DEC-136` records the live Ingest prefix-grouped Digest-unit rule. Historical notes: the larger-preview focused Siddhartha v16 probe failed at unit 7 because of the now-retired final `memory_recalls` language/tool-call mismatch; the v17 source-normalized focused retry completed after the empty-tool-call no-op repair; the 10-book Source Normalization v1.1 validation job was manually stopped after false-positive risk from broad LLM classification; the Digest v16 five-book parallel Marginalia diagnostic completed and remains `review_pending` with `100` runner units and `0` hard failures. No formal A/B rerun or historical report-package regeneration has been done for v19. Next action: run a small focused Ingest smoke on the Siddhartha opening and Naval opening to check whether v19 prefix grouping reduces over-splitting without over-merging.
- Jobs:
