...
Last updated: `2026-06-06T17:50:46+08:00`
...
- Next: five-window Long Span vNext judged diagnostic completed for the current `Ingest -> Digest -> Reading Runner settlement` mechanism and is `review_pending` in the run ledger. Current live `Ingest` returns boundary fields plus bounded `memory_recalls[]`; when recalls are present, the `retrieve_unit_memory` action tool lets Reading Runner execute Unit Memory retrieval/selection before calling `Digest`. `Digest` receives top-level `ReadingMemory` assembled from hot current-chapter Understanding plus selected long-distance Unit Memory Understanding, with `5K` hot, `10K` retrieved, and `15K` total prompt-facing estimated-token budgets. Unit Memory is the current content-neutral long-distance memory substrate under `DEC-110`; retired Detour/source-backread/source-skill, concept/thread structured-memory interfaces, and content-type classifiers are historical only. Diagnostic review found that Unit Memory retrieval was structurally present but initially not mechanism-effective: Digest `ReadingMemory` rendered hot memory, but retrieved long-distance memory had `retrieved_line_count=0` across all `531` Digest calls. The staged repair/validation track in `docs/implementation/new-reading-mechanism/unit-memory-retrieval-repair-validation-plan.md` is now in progress: deterministic phases 1/3/4/5 pass; R9 fixed the whole-active-Recent-Memory exclusion bug; post-R9 `text_only` diagnostic smoke proved prompt-visible retrieved memory; post-R9 relevance review found useful continuity recall but also auxiliary-surface pollution from terminology / note-cluster units; R10 now prioritizes `unit_understanding` lexical weight over source / annotation / response, and R11 records exact rendered retrieved unit ids in selection traces. Post-R10/R11 `text_only` diagnostic smoke (`attentional_v2_unit_memory_text_only_smoke_xidaduo_post_r11_20260606`) was intentionally stopped after rendered-id evidence was collected; its health packet is `ok` with `47` Unit Memory entries, `293` retrieval docs, `selected_unit_count=18`, `renderable_selected_unit_count=11`, `retrieved_line_total=6`, and `rendered_retrieved_unique_unit_count=6`. Ingest prompt `attentional_v2.ingest.v5` / promptset `attentional_v2-phase6-v55` now calibrates recalls toward the selected unit's primary semantic focus and away from broad protagonist / character-background recall unless needed. Post-Ingest-v5 `text_only` smoke (`attentional_v2_unit_memory_text_only_smoke_xidaduo_post_ingest_v5_20260606`) was intentionally stopped after recall-specificity evidence; health is `ok` with `68` Unit Memory entries, `445` retrieval docs, `selected_unit_count=50`, `renderable_selected_unit_count=32`, `retrieved_line_total=27`, and `rendered_retrieved_unique_unit_count=19`. Current selection-discipline repair adds `max_units_per_recall_to_digest_context = 6` with traceable `per_recall_selection_limit_exceeded` suppression; status is `pending_validation`. Next run targeted checks and a small no-judge `text_only` smoke to verify prompt-visible retrieved Understanding remains nonzero while focused-recall retrieved sets become smaller and more directly relevant. Hybrid dense retrieval remains blocked by missing/unreachable local Ollama / Qwen embedding service, so do not claim hybrid success or update evidence catalog.
- Jobs:
