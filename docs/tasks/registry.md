...
Last updated: `2026-06-21T15:57:48+08:00`
...
- Next: Five-window Long Span vNext judged diagnostic remains review_pending in the run ledger, and Unit Memory retrieval repair now has diagnostic proof for both text_only and live hybrid mechanics. Current live Ingest prompt `attentional_v2.ingest.v19` / promptset `attentional_v2-phase6-v76` keeps the token-bounded preview and whole-window `preview_partition[]` audit map, while allowing the committed Digest unit to cover a continuous left prefix `preview_partition[0..k]` ending at one complete partition boundary. Runtime derives private/audit `unit_partition_range`, `unit_partition_titles`, `unit_estimated_token_count`, `unit_size_policy`, and `unit_size_status`; Digest-unit size guidance remains `300 / 900 / 1600` tokens. Unit Memory recall intent is expressed only through `retrieve_unit_memory` action-tool args; runtime derives private/audit `memory_recalls[]` from non-empty tool args, treats empty optional action-tool calls as no-op / not-requested events, and ignores legacy final-output recall echoes. Digest is now live as `attentional_v2.digest.v17` / promptset `attentional_v2-phase6-v77` with unchanged output contract `digest_understanding_response_marginalia_json_v7`: canonical visible-note output is `marginalia[]`; highlight-only Marginalia still uses empty/omitted `content` plus private inline `selection_reason`, but now must pass both an out-of-context completeness gate and a value gate. If a quote's meaning or value collapses without surrounding scene, speaker, character relation, plot function, or prior setup, it should be skipped or become note-bearing Marginalia rather than quote-only. `selection_reason` must name both why the quote remains understandable out of context and what intrinsic excerpt value it carries. Compatibility aliases remain for `annotations[]`, `surfaced_reactions`, legacy link/search metadata, and public `reaction_*` fields. Key decisions through `DEC-137` record the current Ingest/Digest prompt path, including prefix-grouped Ingest unit selection and the tightened highlight-only context-loss rule. Historical diagnostics remain review_pending where noted: the Digest v16 five-book parallel Marginalia diagnostic completed with `100` runner units and `0` hard failures, but its highlight-only quality caveats motivated the v17 prompt refinement. No formal A/B rerun or historical report-package regeneration has been done for v17. Next action: run a small focused Digest v17 Marginalia smoke against the prior good/bad highlight-only examples and at least one real dataset opening unit.
- Jobs:
