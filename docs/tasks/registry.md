...
Last updated: `2026-06-21T14:50:51+08:00`
...
- Next: Five-window Long Span vNext judged diagnostic remains review_pending in the run ledger, and Unit Memory retrieval repair now has diagnostic proof for both text_only and live hybrid mechanics. Current live Ingest prompt `attentional_v2.ingest.v18` / promptset `attentional_v2-phase6-v75` uses the reviewed bounded-lookahead window-partition selector plus the live `preview_partition[]` audit map, and now adds the same-local-function boundary rule so adjacent setup, character construction, scene build, argument support, example chains, or emotional turns are not split merely because smaller facets can be titled separately: final output returns `unit.end_paragraph_n` / `unit.end_at`, `preview_partition[]`, and optional first-unit boundary `reason`; `preview_partition[0]` must match `unit`, while later entries are compact future-source planning/audit metadata only. Unit Memory recall intent is expressed only through `retrieve_unit_memory` action-tool args; runtime derives private/audit `memory_recalls[]` from non-empty tool args, treats empty optional action-tool calls as no-op / not-requested events, and ignores legacy final-output recall echoes. Runtime now builds the visible Ingest preview with token-bounded, paragraph-aligned assembly: `preview_soft_min_tokens=1600`, `preview_target_max_tokens=3000`, `preview_hard_max_tokens=4200`, `emergency_max_preview_paragraphs=200`, and old char-budget / paragraph-count snapshots ignored as normal stopping rules. Digest is now live as `attentional_v2.digest.v16` / promptset `attentional_v2-phase6-v74` inside active bundle `attentional_v2-phase6-v75`: the canonical visible-note output is `marginalia[]`, each item requires `source_quote` and accepts optional/empty `content` for highlight-only marks, with highlight-only reserved for intrinsically valuable excerpt-worthy quotes that can stand alone rather than merely complete, structural, or context-dependent spans; highlight-only Marginalia carries one private inline `selection_reason`, note-bearing Marginalia keeps its reason in visible `content`, and every `source_quote` should be the smallest complete contiguous local meaning span. `annotations[]`, `surfaced_reactions`, legacy link/search metadata, and public `reaction_*` fields are compatibility aliases for historical artifacts and existing API clients. New Unit Memory entries store `digest.marginalia` and derive `unit_marginalia` retrieval docs, while old `unit_annotation` docs remain readable. Runtime derives `preview_partition_audit[]` in `UnitizeDecision`, `unitization_audit.jsonl`, and compact ingest traces where partition boundaries resolve; later unresolved or non-advancing partitions mark audit partial and do not affect the accepted Digest unit. `retrieve_unit_memory` remains the only live action tool, keeps its recall schema, validates same-language / `selected_source_unit` basis for non-empty recall targets during tool preflight, treats empty calls as no-op, and does not require `preview_partition[]`. `DEC-116` records why bounded lookahead improves first-unit selection, `DEC-117` records the live preview-partition audit contract, `DEC-118` records the paragraph-count to character-bound repair, `DEC-120` records the live token-bounded preview plus v16 output-discipline change, `DEC-121` records the larger v16 preview-capacity calibration, `DEC-122` through `DEC-125` record the Source Normalization direction through deterministic-only v1.2, `DEC-127` records the recall-authority cleanup, `DEC-128` records the Marginalia terminology/contract migration, `DEC-129` records the Marginalia v11 prompt/output-contract simplification, `DEC-130` records the Marginalia v12 highlight-only excerpt-worthiness boundary, `DEC-131` records the intrinsic excerpt-value gate, and `DEC-132` records the private highlight-selection audit reason, `DEC-133` records moving that reason inline to `marginalia[].selection_reason`, `DEC-134` records the smallest-complete-span quote-selection rule, and `DEC-135` records the same-local-function Ingest boundary rule. The larger-preview focused Siddhartha probe `bgjob_ingest_live_v16_token_preview_larger_to_old_unit13_20260613` failed at unit 7 with `llm_contract` after provider calls returned ok; that historical failure came from the now-retired final `memory_recalls` language/tool-call mismatch. The 10-book Source Normalization v1.1 validation job was manually stopped after partial review exposed unacceptable false-positive risk from broad LLM source-flow classification. The v17 source-normalized dataset-window focused probe `bgjob_ingest_live_v17_source_norm_token_preview_to_chapter1_20260614` failed at unit 2 with `llm_contract` after provider calls returned ok because runtime treated an empty `retrieve_unit_memory(memory_recalls=[])` action-tool call as fatal; the focused retry `bgjob_ingest_live_v17_source_norm_token_preview_to_chapter1_retry1_20260614` completed after the empty-tool-call no-op repair on the same active `xidaduo_private_zh__segment_1` scope through `P57@2`, with `stop_reason=target_reached` and `unit_count=10`. No formal A/B rerun or historical report-package regeneration has been done for v18. Digest v16 five-book parallel Marginalia diagnostic `bgjob_digest_marginalia_v16_5book_parallel_20units_20260621` completed and is `review_pending` in the run ledger: `100` runner units, `123` Marginalia items, `99` highlight-only, `24` note-bearing, `13` `quote_too_broad` caveats, and `0` hard failures. Next action: rerun a small focused Ingest smoke on the Siddhartha opening and Naval opening to check whether v18 reduces over-splitting without over-merging.
- Jobs:
