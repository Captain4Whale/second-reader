...
Last updated: `2026-06-28T16:05:03+08:00`
...
- Next: Digest is live as `attentional_v2.digest.v19` / promptset `attentional_v2-phase6-v79` with unchanged output contract `digest_understanding_response_marginalia_json_v7`: canonical visible-note output is `marginalia[]`; highlight-only Marginalia still uses empty/omitted `content` plus private inline `selection_reason`, and must pass both out-of-context completeness and durable long-term reader value gates. Note-bearing Marginalia now asks for useful knowledge, context, or non-obvious connections that a thoughtful ordinary reader may not know, notice, or infer on their own; ordinary close-reading / technique commentary is explicitly demoted unless it creates non-obvious gain. The Digest v19 five-book parallel diagnostic `digest_marginalia_v19_5book_parallel_20units_20260621` failed after `61` runner units: `huochu_shengming_de_yiyi_private_zh__segment_1` hit `llm_contract` after `5` accepted units and `nawaer_baodian_private_zh__segment_1` hit `llm_timeout` after `1` accepted unit; `xidaduo` and `mangge` reached the `20`-unit limit, and `value_of_others` reached `chapter_end` after `15` units. The partial report exists at `reading-companion-backend/eval/runs/attentional_v2/digest_marginalia_v19_5book_parallel_20units_20260621/analysis/digest_marginalia_v19_5book_parallel_20units/marginalia_smoke_report.md`; next action is to inspect the `huochu` contract failure and the `nawaer` timeout before treating v19 quality results as representative.
- Jobs:
