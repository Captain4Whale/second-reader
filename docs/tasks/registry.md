...
Last updated: `2026-06-06T21:05:29+08:00`
...
- Next: Five-window Long Span vNext judged diagnostic completed for the current Ingest -> Digest -> Reading Runner settlement mechanism and is review_pending in the run ledger. Current live Ingest returns boundary fields plus bounded memory_recalls[]; when recalls are present, retrieve_unit_memory lets Reading Runner execute Unit Memory retrieval/selection before Digest. Digest receives top-level ReadingMemory assembled from hot current-chapter Understanding plus selected long-distance Unit Memory Understanding, with 5K hot, 10K retrieved, and 15K total prompt-facing estimated-token budgets. Unit Memory is the current content-neutral long-distance memory substrate under DEC-110; retired Detour/source-backread/source-skill, concept/thread structured-memory interfaces, and content-type classifiers are historical only. Diagnostic review found that Unit Memory retrieval was structurally present but initially not mechanism-effective: Digest ReadingMemory rendered hot memory, but retrieved long-distance memory had retrieved_line_count=0 across all 531 Digest calls. The staged repair/validation track in docs/implementation/new-reading-mechanism/unit-memory-retrieval-repair-validation-plan.md is now in progress: deterministic phases 1/3/4/5 pass; text-only smokes after R9/R11 proved prompt-visible retrieved memory; Ingest v5 reduced one broad recall case; the post-selection-cap smoke (attentional_v2_unit_memory_text_only_smoke_xidaduo_post_selection_cap_20260606) validated max_units_per_recall_to_digest_context = 6 with per_recall_selection_limit_exceeded=8, retrieved_line_total=2, and rendered_retrieved_unique_unit_count=2. Ingest prompt attentional_v2.ingest.v6 / promptset attentional_v2-phase6-v56 now has code-enforced recall language/basis validation: final-output validation and retrieve_unit_memory action-tool preflight reject clear cross-language model-side recalls and non-selected_source_unit basis values. The pre-contract smoke attentional_v2_unit_memory_text_only_smoke_xidaduo_post_ingest_v6_20260606 exposed Chinese-source -> English recall drift; the post-contract smoke attentional_v2_unit_memory_text_only_smoke_xidaduo_post_ingest_v6_contract_20260606 observed zero reviewed language violations and only selected_source_unit basis values, but stopped before a mature long-distance retrieval horizon. Prompt-visible hot-memory exclusion is now validated in text_only diagnostics: Reading Runner excludes only hot current-chapter spans that would already render in Digest ReadingMemory, does not exclude the whole active Recent Reading Memory store, and records excluded_source_unit_span_count; the post-hot-exclusion smoke rendered 23 retrieved lines from 11 unique units with selected_but_not_rendered_count=0, excluded_source_unit_span_total=250, max_excluded_source_unit_span_count=36, and dedupe_hot_memory=0. Post-hot-exclusion relevance review found the next repair target: after hot candidates are correctly excluded, selection can still backfill long-distance slots with low-score or broad weak memories. R13 selection-quality gating is now validated in text_only diagnostics with a boundary caveat: candidates need strong enough unit_understanding evidence or stricter auxiliary-surface evidence, weak candidates are suppressed with candidate_below_selection_quality_threshold, and the post-R13 smoke attentional_v2_unit_memory_text_only_smoke_xidaduo_post_r13_20260606 showed selected_unit_count=6, retrieved_line_total=6, rendered_retrieved_unique_unit_count=6, selected_but_not_rendered_count=0, and candidate_below_selection_quality_threshold=11; review then found one selected event attached to a one-character closing-quote fallback unit caused by straight-vs-curved quote anchor mismatch. R14 quote-normalized exact anchor resolution is live-smoked in the post-R14 diagnostic: the run observed 4 normalized_exact_text matches and did not reproduce the old p125 one-character fallback, but it exposed R15, a trailing-closing-quote remainder unit caused by successful anchors that stop immediately before a closing quote. R15 trailing-closing-punctuation extension is live-smoked in the post-R15 diagnostic: the run observed 9 live extensions, tiny_unit_count=0, and the prior p116 failure region resolved as full unit src:c1:p115@0-p116@42. The next target is a mature post-R15 text_only retrieval sample before judging post-R13 selected-memory relevance. Do not rerun solely for the v6 language/basis contract unless later live artifacts show drift again. Phase 2 is now partially validated at the code-path layer: sqlite-vec loads, fake-embedder coverage proves vector rows, query embedding cache, dense candidates, dense_max_distance filtering, and dense-channel selection; `scripts/check_unit_memory_hybrid_readiness.py` now provides the repeatable operator check. The latest local readiness probe reports sqlite-vec ok but `blocking_reasons=[ollama_unreachable]`. Live hybrid dense retrieval remains blocked by missing/unreachable local Ollama / Qwen embedding service, so do not claim live hybrid success or update evidence catalog.
- Jobs:
...
- Next: continue implementation from the current `Ingest -> Digest -> Reading Runner settlement` baseline:
  ...
  - Paragraph-offset mainline cursor and SourceRef cutover are now landed:
    ...
    - `Ingest` receives an adaptive paragraph-offset preview and returns exact `end_anchor_text`
