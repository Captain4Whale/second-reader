...
- Current prompt assembly separates the product-level `ReaderRole` from per-call `Instruction`.
  ...
  - The current Ingest output contract is submitted through the `submit_ingest_result` final-output tool with `end_anchor_text`, boundary-rationale `reason`, and bounded `memory_recalls[]`.
