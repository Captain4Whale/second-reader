...
- Shared substrate dependency
  ...
  - `Source Normalization` is now live for newly created parsed-book documents. It attaches richer `source_normalization` metadata to existing paragraph records rather than introducing persistent `reading_blocks[]`; raw paragraph coordinates remain the highlight/source-reference authority. Markup-aware v1.1 also preserves ancestor container and inline-anchor metadata, skips duplicate parent containers such as `blockquote > p` aggregates, applies deterministic exclusion for explicit footnote/endnote/note-definition evidence, and protects blockquote/poem/verse正文 from unbacked `layout_noise` labels. Only high-confidence auxiliary/reference/noise/front-back/caption-support labels with structural evidence change the coarse `text_role` gate, and LLM failures degrade to deterministic parse roles instead of failing parse. The design and follow-ups live in `docs/implementation/new-reading-mechanism/source-normalization-design.md`.
