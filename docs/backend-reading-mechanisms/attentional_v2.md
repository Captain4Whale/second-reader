...
- Current prompt assembly separates the product-level `ReaderRole` from per-call `Instruction`.
  ...
  - Digest prompt version `attentional_v2.digest.v19` / promptset `attentional_v2-phase6-v79` uses content-level Understanding rules, text-type compression guidance, grammatical-subject guidance, source-established-content calibration, subject-continuity rules, approved examples, and Marginalia-specific page-margin wording to keep stored Understanding memory self-contained while keeping visible marks source-anchored and reader-facing.
  - Digest v19 keeps the visible Marginalia item shape from v11-v18 while recalibrating quality: highlight-only marks must be durable out-of-context excerpts with long-term reader value, not merely local book evidence; note-bearing content should add useful knowledge, context, or non-obvious connections that a thoughtful ordinary reader may not know, notice, or infer on their own. Ordinary close-reading / technique commentary is demoted unless it reveals something non-obvious and materially changes the quote's value. Private `selection_reason` for highlight-only items names both completeness and durable excerpt value.
...
- Subject continuity is implemented in Digest prompt `attentional_v2.digest.v19` and documented in `docs/implementation/new-reading-mechanism/ingest-recall-and-digest-memory-context-design.md`.
