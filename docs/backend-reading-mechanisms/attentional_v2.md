...
- Current prompt assembly separates the product-level `ReaderRole` from per-call `Instruction`.
  ...
  - Ingest prompt version `attentional_v2.ingest.v5` keeps recalls reader-shaped while steering them toward the selected unit's primary semantic focus; it avoids broad protagonist / character-background recall unless the current unit needs that earlier relation, choice, conflict, or pressure, and it permits empty recalls when only generic memory would be requested.
