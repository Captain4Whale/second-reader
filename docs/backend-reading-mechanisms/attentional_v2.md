...
- Current prompt assembly separates the product-level `ReaderRole` from per-call `Instruction`.
  ...
  - Digest prompt version `attentional_v2.digest.v16` / promptset `attentional_v2-phase6-v74` uses content-level Understanding rules, text-type compression guidance, grammatical-subject guidance, source-established-content calibration, subject-continuity rules, approved examples, and Marginalia-specific page-margin wording to keep stored Understanding memory self-contained while keeping visible marks source-anchored and reader-facing.
  - Digest v16 keeps the visible Marginalia item shape from v11-v15 while tightening quote-span selection: quote-only Marginalia should be excerpt-worthy spans whose value is visible in the quoted words themselves, self-contained is necessary but not sufficient, each highlight-only item must have a short mechanism-private `selection_reason`, and every Marginalia `source_quote` should be the smallest complete contiguous local meaning span rather than a clipped phrase or isolated term.
...
- Subject continuity is implemented in Digest prompt `attentional_v2.digest.v16` and documented in `docs/implementation/new-reading-mechanism/ingest-recall-and-digest-memory-context-design.md`.
