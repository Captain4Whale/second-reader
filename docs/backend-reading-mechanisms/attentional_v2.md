...
- Current prompt assembly separates the product-level `ReaderRole` from per-call `Instruction`.
  ...
  - Digest prompt version `attentional_v2.digest.v13` / promptset `attentional_v2-phase6-v71` uses content-level Understanding rules, text-type compression guidance, grammatical-subject guidance, source-established-content calibration, subject-continuity rules, approved examples, and Marginalia-specific page-margin wording to keep stored Understanding memory self-contained while keeping visible marks source-anchored and reader-facing.
  - Digest v13 keeps the v5 Marginalia output contract while tightening highlight-only selection: quote-only Marginalia should be excerpt-worthy spans whose value is visible in the quoted words themselves, and self-contained is necessary but not sufficient. Structurally important but context-dependent topic sentences, transitions, setup questions, recaps, and argument signposts should usually stay in Understanding or become note-bearing Marginalia rather than quote-only highlights.
...
- Subject continuity is implemented in Digest prompt `attentional_v2.digest.v13` and documented in `docs/implementation/new-reading-mechanism/ingest-recall-and-digest-memory-context-design.md`.
