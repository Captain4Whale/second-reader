...
- Current prompt assembly separates the product-level `ReaderRole` from per-call `Instruction`.
  ...
  - Digest prompt version `attentional_v2.digest.v15` / promptset `attentional_v2-phase6-v73` uses content-level Understanding rules, text-type compression guidance, grammatical-subject guidance, source-established-content calibration, subject-continuity rules, approved examples, and Marginalia-specific page-margin wording to keep stored Understanding memory self-contained while keeping visible marks source-anchored and reader-facing.
  - Digest v15 keeps the visible Marginalia item shape from v11-v13 while moving private highlight-only audit reasons inline: quote-only Marginalia should be excerpt-worthy spans whose value is visible in the quoted words themselves, self-contained is necessary but not sufficient, and each highlight-only item must have a short mechanism-private `selection_reason`.
...
- Subject continuity is implemented in Digest prompt `attentional_v2.digest.v15` and documented in `docs/implementation/new-reading-mechanism/ingest-recall-and-digest-memory-context-design.md`.
