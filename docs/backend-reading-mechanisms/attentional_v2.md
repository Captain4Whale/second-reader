...
- Current prompt assembly separates the product-level `ReaderRole` from per-call `Instruction`.
  ...
  - Digest prompt version `attentional_v2.digest.v17` / promptset `attentional_v2-phase6-v77` uses content-level Understanding rules, text-type compression guidance, grammatical-subject guidance, source-established-content calibration, subject-continuity rules, approved examples, and Marginalia-specific page-margin wording to keep stored Understanding memory self-contained while keeping visible marks source-anchored and reader-facing.
  - Digest v17 keeps the visible Marginalia item shape from v11-v16 while tightening highlight-only selection: quote-only Marginalia must pass both an out-of-context completeness gate and a value gate. If a quote's importance collapses without knowing the surrounding scene, speaker, character relation, plot function, or prior setup, it should be skipped or made note-bearing rather than highlight-only. Each highlight-only item must have a short mechanism-private `selection_reason` naming both completeness and value, and every Marginalia `source_quote` should be the smallest complete contiguous local meaning span rather than a clipped phrase or isolated term.
...
- Subject continuity is implemented in Digest prompt `attentional_v2.digest.v17` and documented in `docs/implementation/new-reading-mechanism/ingest-recall-and-digest-memory-context-design.md`.
