...
- Current prompt assembly separates the product-level `ReaderRole` from per-call `Instruction`.
  ...
  - Digest prompt version `attentional_v2.digest.v18` / promptset `attentional_v2-phase6-v78` uses content-level Understanding rules, text-type compression guidance, grammatical-subject guidance, source-established-content calibration, subject-continuity rules, approved examples, and Marginalia-specific page-margin wording to keep stored Understanding memory self-contained while keeping visible marks source-anchored and reader-facing.
  - Digest v18 keeps the visible Marginalia item shape from v11-v17 while tightening note-bearing quality: note-bearing content should answer what a thoughtful ordinary reader may not notice, know, or infer on their own, adding cognitive value through hidden structure, literary technique, motif, cultural/historical context, translation nuance, philosophical tension, precise inference, or a question that changes how the quote is read. Shallow paraphrase, obvious emotion labels, and visible-action restatement should be skipped. Digest v17's highlight-only completeness/value gates remain active: quote-only Marginalia must remain understandable when lifted out of the book and carry intrinsic excerpt value, with private `selection_reason` naming both completeness and value.
...
- Subject continuity is implemented in Digest prompt `attentional_v2.digest.v18` and documented in `docs/implementation/new-reading-mechanism/ingest-recall-and-digest-memory-context-design.md`.
