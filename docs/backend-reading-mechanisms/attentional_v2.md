...
- Current prompt assembly separates the product-level `ReaderRole` from per-call `Instruction`.
  ...
  - Ingest prompt version `attentional_v2.ingest.v8` keeps recalls reader-shaped while making them retrieval-friendly: each recall should name the earlier understanding runtime should try to find, look backward beyond the selected unit rather than summarize the selected unit itself, avoid paragraph / line / XML identifiers, avoid mechanical noun lists, prefer one strong recall over several weak ones, use the current source text's primary language, and keep model-side recall `basis` exactly `selected_source_unit`.
