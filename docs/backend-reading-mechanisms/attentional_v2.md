...
- The post-F4 cleanup has also retired the old gate/pressure sidecar from current state.
  ...
  - Unit Memory selection also applies a content-neutral quality gate after aggregation and renderability checks: candidates need strong enough `unit_understanding` evidence, or a stricter auxiliary-surface signal, before they can occupy long-distance `ReadingMemory` slots. Weak candidates are suppressed with `candidate_below_selection_quality_threshold` instead of filling remaining budget by default.
...
- Current prompt assembly separates the product-level `ReaderRole` from per-call `Instruction`.
  ...
  - Ingest prompt version `attentional_v2.ingest.v6` keeps recalls reader-shaped while steering them toward the selected unit's primary semantic focus; it avoids broad protagonist / character-background recall unless the current unit needs that earlier relation, choice, conflict, or pressure, permits empty recalls when only generic memory would be requested, requires recall text to use the current source text's primary language, and requires model-side recall `basis` to be exactly `selected_source_unit`.
