...
- Runtime validation also now requires final submitted `memory_recalls[]` to match the `retrieve_unit_memory` action-tool recalls when the tool was called; a v10 probe exposed that the model could otherwise call the tool with one recall set and finalize with an empty recall list.
