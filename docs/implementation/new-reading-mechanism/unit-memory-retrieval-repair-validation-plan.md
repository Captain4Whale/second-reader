...
- The local live hybrid path is no longer blocked at the environment-readiness layer: the Ollama App runtime is installed, `qwen3-embedding:0.6b` is present, the embedding probe returns `1024` dimensions, and sqlite-vec can import/load/create a `vec0` table.
- `scripts/check_unit_memory_hybrid_readiness.py` now provides a repeatable operator-facing Phase 2 readiness probe. Current local output: `status = ok`; the next unresolved proof is a live hybrid retrieval smoke showing real query-embedding cache rows, vector rows, dense candidates, and RRF contribution in current Unit Memory artifacts.
...
- The Ingest action-tool preflight validates recall language against the current source text before retrieval execution. If the model submits a cross-language recall or other contract-violating recall payload through `retrieve_unit_memory`, the tool returns `contract_violation` metadata so the forced final-output path can repair the result instead of silently retrieving on a bad recall.
...
- Ingest prompt `attentional_v2.ingest.v11` / promptset `attentional_v2-phase6-v61` recalibrates `RecallPriorReading` from a prior-fact assertion shape into a retrieval-target shape and adds a `Boundary closure check`: each recall should describe what earlier understanding runtime should try to find from selected-unit cues, without requiring the model to assert that prior memory already exists. It treats `CurrentView / Content` as boundary/recall-cue preview rather than already-read memory evidence, looks backward beyond the selected unit, avoids paragraph / line / XML identifiers, avoids noun-list splitting, prefers one strong recall over several weak recalls, retrieves relevant continuity even when the current unit is locally intelligible, and reminds Ingest not to leave terminal punctuation / attached closing marks behind when they complete the chosen unit.
...
- Ingest v8 recall wording is the current calibration target. It needs lightweight live probe review for recall text shape, especially whether recall text avoids paragraph identifiers, names a retrievable prior understanding rather than a source-location cue, and avoids treating the selected unit itself as the prior memory target.
...
- Validate the retrieval chain:
  - Ingest calls `retrieve_unit_memory` with bounded `memory_recalls[]` args
...
### R14. Quote-mark anchor mismatch can create a tiny fallback source unit

Symptom:

- The post-R13 review found one selected retrieval event whose `accepted_source_span_id` was `src:c1:p125@63-p125@64`, a one-character closing quotation mark.
- Ingest had intended the next semantic unit ending at `"看！"悉达多轻声对乔文达道，"此人就是佛陀。"`, but the source text used Chinese curved quotation marks: `“看！”悉达多轻声对乔文达道，“此人就是佛陀。”`.
- Exact anchor resolution failed, retry did not recover a source-exact quote, and deterministic fallback accepted the next available cursor boundary. Digest then read a structural quote instead of the intended scene.

Root cause:

- `resolve_end_anchor_text(...)` treated straight quotes and curved quotes as different characters.
- The fallback path is intentionally conservative for unresolved anchors, but in this case the mismatch was a punctuation-equivalence issue, not a semantic/paraphrase mismatch.

Repair:

- Keep exact matching as the first path.
- If exact matching fails, run a quote-normalized exact match that treats straight, curved, and full-width quote marks as equivalent while preserving the original source offset mapping.
- Do not add semantic fuzzy matching or paraphrase acceptance.

Validation:

- `fixed_deterministic`: `tests/test_attentional_v2_source_spans.py::test_resolver_matches_anchor_with_equivalent_quote_marks` verifies an ASCII-quote anchor resolves against curved-quote source text and returns the true end cursor.
- `validated_text_only_diagnostic_with_followup_boundary_issue`: the post-R14 smoke `attentional_v2_unit_memory_text_only_smoke_xidaduo_post_r14_20260606` observed `4` live `normalized_exact_text` matches and did not reproduce the old `src:c1:p125@63-p125@64` fallback. It should not be used for retrieval relevance because it was stopped early and selected no long-distance memory.

### R15. Successful anchor can leave an adjacent closing quote as a one-character remainder

Symptom:

- The post-R14 smoke found a new quote-only Unit Memory entry: `U37`, `src:c1:p116@41-p116@42`, with source text `”`.
- The previous unit ended at the exact anchor `还是让那些沙门老朽为这些把戏沾沾自喜吧！`, leaving the immediately adjacent closing quote outside the accepted unit.
- The next Ingest attempt selected a later semantic boundary, but the anchor did not resolve and fallback accepted the current one-character paragraph remainder.

Root cause:

- Successful exact / quote-normalized anchor matching previously accepted the cursor exactly at the end of the matched anchor text.
- When the model omitted a closing quote that was immediately adjacent to the anchor, runtime created an artificial structural remainder that became a future source unit.

Repair:

- Keep exact matching and quote-normalized exact matching as the only source-boundary match modes.
- After a successful match, extend the accepted flat offset over immediately adjacent trailing closing punctuation, then map the extended offset back to the true source cursor.
- Do not add semantic fuzzy matching or paragraph-skipping behavior for this repair.
...
- Lightweight v8 live probes did not show paragraph/line/XML identifiers inside `recall_text`, but still exposed mixed recall quality: one probe invented or misassigned a speaker name, and some recalls summarized the selected unit itself rather than prior reading. Treat v8 as a cleaner prompt shape, not as validated recall-quality closure.
- Lightweight v9 live probes improved the naming problem but still showed selected-unit / preview self-summary: Ingest treated `CurrentView` text as prior-memory content. This led to the v10 `Source scope` rule.
- Ingest prompt `attentional_v2.ingest.v10` / promptset `attentional_v2-phase6-v60` keeps the same internal markdown headings but changes the purpose from "focused memory intention" to "retrieval target for prior reading memory" and adds `Source scope`.
- Runtime validation also required final submitted `memory_recalls[]` to match the `retrieve_unit_memory` action-tool recalls when the tool was called; a v10 probe exposed that the model could otherwise call the tool with one recall set and finalize with an empty recall list. This historical matching rule is superseded by `DEC-127`: action-tool args are now the only recall authority, and final-output recall echoes are ignored.
- `pending_validation`: v10 should produce recall texts that are usable as retrieval queries without paragraph/line/XML identifiers, noun-list splitting, selected-unit or future-preview self-summary, invented speaker names, or search-interface wording.
- Lightweight v10 post-contract probes showed cleaner behavior than v8/v9: final recalls matched the action-tool recalls, no paragraph/line/XML identifiers appeared, and the most obvious speaker-name invention was avoided. Remaining review risk: some recall texts still echo selected-unit concepts as broad retrieval targets, so mature run artifacts should check whether those queries retrieve genuinely useful prior Understanding rather than simply reinforcing the current unit.

Post-v11 boundary-closure follow-up:

- Ingest prompt `attentional_v2.ingest.v11` / promptset `attentional_v2-phase6-v61` adds a `Boundary closure check` inside `SelectNextUnit`.
- The rule asks Ingest to include terminal punctuation and attached closing marks that complete the chosen semantic unit, such as sentence-final punctuation and closing quotes/brackets.
- The same rule explicitly tells Ingest not to absorb opening punctuation, bullets, separators, or markers that begin the next unit.
- This is intentionally a model-side boundary calibration, not a broad runtime auto-absorption rule; runtime should still reject or govern non-content accepted units separately if later implementation addresses symbol-only residue.
