...
- Ingest prompt `attentional_v2.ingest.v8` / promptset `attentional_v2-phase6-v58` recalibrates `RecallPriorReading` from a mostly negative/broadness-avoidance instruction into a retrieval-friendly recall intention: each recall should name the earlier understanding runtime should try to find, look backward beyond the selected unit rather than summarize the selected unit itself, avoid paragraph / line / XML identifiers, avoid noun-list splitting, prefer one strong recall over several weak recalls, and retrieve relevant continuity even when the current unit is locally intelligible.
...
- Ingest prompt `attentional_v2.ingest.v8` / promptset `attentional_v2-phase6-v58` reframes `RecallPriorReading` with internal markdown headings: Purpose, When to recall, Retrieval-friendly content, Focus, Writing constraints, Number of recalls, and Tool use.
- `pending_validation`: v8 should produce recall texts that are usable as retrieval queries without paragraph/line/XML identifiers, noun-list splitting, selected-unit self-summary, invented speaker names, or search-interface wording.
