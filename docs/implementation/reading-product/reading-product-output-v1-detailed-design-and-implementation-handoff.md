...
## 10. Production-scale Xidaduo live acceptance

`TASK-READING-PRODUCT-XIDADUO-FULL-BOOK-LIVE-ACCEPTANCE` separately closes the production-scale technical Gate on a new run of the exact retained《悉达多》EPUB, SHA-256 `f239921773ac5abc86527fb78379cbd68cdf2cb901d253e085b2883180984a4f`. It used ordinary `parse_book` / `read_book`, current `attentional_v2`, real `cpa_codex_local` / `gpt-5.6-luna` calls, `reasoning_effort=medium`, and `hybrid` memory retrieval. It did not reuse or rewrite a historical evaluation output.

- The fresh attempt committed `78` Product Units before one upstream TLS handshake EOF exhausted its three physical attempts. Pre-resume checks found the source hash, partial Product, Product Store ledger, cursor, and reading ID coherent. Normal `continue_mode=True` resumed reading ID `urn:uuid:f91e6d64-0d70-48e7-a18e-95d7db6504fa` without repeating committed model work and reached runtime `completed`.
- The final immutable Reading Product revision is `543dd6b2c8338b601a14620dcfc8d0cc9aad7b794d59abb7e21ec727f1abf99f`: `89` Units, non-empty Understanding/Response in every Unit, and `207` Marginalia (`89` Highlights, `118` Notes) over the real `16`-chapter / `590`-paragraph BookDocument. Product Units produce `15` chapter compatibility projections.
- The local LLM ledger contains `325` logical call rows: `324` successful rows with complete usage and the one recovered `network_blocked` row with usage unavailable. Known successful usage totals `2,034,866` input, `159,465` output, `50,752` reasoning, and `2,194,331` total tokens. The profile uses subscription billing, so `actual_billed_cost` remains unknown; the recorded reference-rate usage value is `$0.58100512`, not an actual charge.
- The default adapter published detached Pack revision `b96c08f4322d9370d25485d3b1abadd1f8ceada47c1c74031c9ba68cb9099c5b`, semantic digest `2e21ed225e7e47a21d003a2f5f403ad8ff0c6599ccc7566a018b3b2c1e2a74bd`, with all `207` Product Marginalia and no skips or errors. Every TextQuote/TextPosition anchor round-trips against one of `22` verified EPUB resources; there are zero unverifiable hrefs. Standalone package validation and inspection both pass, and two subsequent exports return byte-stable `unchanged`.
- The producer validation report retains one nonfatal `invalid_publication_identifier` warning because the source OPF explicitly contains an invalid optional ISBN which is omitted. Minimal Pack identity continues to use the exact EPUB NIH hash; detached semantic validation has zero findings. During close-out, the validation catalog and upstream-warning replay omission that initially rejected/recomputed this warning were fixed with a repeat-export regression in `237bbba`.
- Terminal verification includes the registered no-model `validation2` job, standalone validate/inspect, `make reading-product-contract-check`, `make annotation-pack-contract-check` (`55 passed`), and the combined Annotation Pack / Reading Product / current-mechanism Product regression set (`836 passed`).

This permits the bounded claim: **在固定 SHA-256 的真实《悉达多》EPUB 上，当前机制已经通过 CPA Luna 的真实 LLM 调用完成整书阅读，并生成 complete Reading Product、章节兼容投影和独立验证通过的 207-item Annotation Pack。** It proves the current production-scale technical lifecycle and recovery seam, not human-reviewed reading quality, conversion of old Xidaduo artifacts, public Pages availability, a native Unit API, frontend Understanding/Response, or Library/API/Reader integration. Canonical run evidence is indexed in `reading-companion-backend/docs/evaluation/run_ledger.json` and rooted locally at `reading-companion-backend/state/reading_product_live_acceptance/cpa_luna_xidaduo_full_book_20260903/`.
