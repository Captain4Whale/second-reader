...
外部 IRI 仍须等待 workflow 进入 `main`、Pages 启用并成功部署后做 HTTP byte comparison，当前不得称为 live。下一实现单元是 **Slice 7 — Detached package generation**；它必须把canonical `annotations.json`封装为可独立验证的正式`.annotations`，保持Slice 6 JSON-only revision immutable，并在同一publication revision/pointer模型下以新的完整deliverable-set revision发布。

### Explicit non-goals confirmation

截至 Slice 6，仓库已实现 contract/schema/context/examples、producer-neutral 离线 schema loader、deterministic generated bindings/runtime resources、focused checks/Pages projection、verified EPUB publication identity/fingerprints/coherence gate、exact XHTML resource index、strict anchors、canonical serialization/semantic digest、generic Pack builder、bounded schema/semantic/privacy validator/final report primitives、只读current native settled ledger的strict producer adapter，以及显式JSON exporter/validator/safe inspector与immutable public revision/atomic pointer publication。尚未实现正式detached `.annotations` package、package validator/inspector、真EPUB golden end-to-end export或catalog/API/UI discovery。Default CFI resolver仍不存在，因此不声称真实 CFI互操作。Agent prompt、Digest、Memory、reading loop、Readest、Library 和 public HTTP API 均未修改。
