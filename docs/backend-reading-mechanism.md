...
- Shared substrate
  ...
  - Source normalization is the upstream shared-substrate responsibility for classifying original paragraph/block records into mainline, heading, auxiliary, and noise roles before one mechanism reads them. Live v1 applies to newly created parsed-book documents, attaches `source_normalization` metadata to existing paragraph records, and still uses `text_role == "auxiliary"` as the coarse mainline exclusion gate. The design and follow-ups live in `docs/implementation/new-reading-mechanism/source-normalization-design.md`.
...
- Shared runtime shell
  ...
  - The current CPA / Luna Medium function-tool policy plus historical OpenCode / DeepSeek and MiniMax transport notes live in `docs/implementation/new-reading-mechanism/llm-structured-output-protocol-note.md`.
