...
**Boundary**: This decision approved the design and implementation seam; implementation remains bounded to the contract, producer-neutral reference implementation, explicit exporter, and public artifact path. It does not modify Agent prompts, Digest, Memory, settlement, the reading loop, public HTTP APIs, Readest, Library, evaluation, or automatic completion behavior. Full fuzzy cross-edition or cross-translation alignment remains outside v0.
...
**Decision / Inflection**: Make the Annotation Pack producer handoff explicitly mechanism-neutral, separating source-publication facts, annotation intent, runtime settlement facts, and exporter-derived wire data.

**Period**: August 25, 2026, immediately after minimal v0 closure, when discussion of whether a recent Agent output was convertible exposed that the current `attentional_v2-phase9` adapter gate could be mistaken for the enduring Pack input contract.

**Problem**: The public Pack must identify an exact EPUB and encode Highlights/Notes against its text, but most of those values should never be generated by an LLM. EPUB hash/title/authors, manifest hrefs, resource-wide TextPosition offsets, W3C fields, deterministic IDs, and packaging can be verified or derived. Conversely, the exporter must not invent the mechanism's annotation choice: which exact source span is selected, whether it is a Highlight or Note, and what the Note says. Treating `attentional_v2-phase9` as the universal contract would couple future mechanisms to one private ledger and confuse current adapter compatibility with public Pack capability.

**Decision**: The canonical responsibility model lives in `contract/annotation-pack/v0/README.md`. Verified EPUB/parser substrate owns book/file identity, metadata, manifest resources, deterministic resource text, and `BookDocument` coherence. A reading mechanism as a whole owns the normalized annotation decision: `kind`, one exact shared-`BookDocument` source range and quote, and conditional non-empty Note content. Reading runtime/settlement owns the accepted annotation creation time. A mechanism adapter maps those values into producer-neutral `AnnotationDraft`; adapter record index/digest and ledger provenance remain private control metadata. The generic resolver/builder/exporter owns href/TextQuote/TextPosition derivation, W3C shaping, generator/generated values, deterministic IDs, ordering, validation, serialization, packaging, and local pointer/report companions.

`attentional_v2-phase9` remains the only private ledger accepted by the current `SecondReaderProducerAdapter`, but it is not a Pack version or a required neutral field. A future mechanism may use a different private ontology, prompt, citation representation, ledger, and version; its dedicated adapter must emit the same neutral handoff instead of pretending to be phase9. Any product-default mechanism that emits user-visible annotations must document and verify this mapping. A fallback or experimental mechanism without an adapter must state Annotation Pack export is unsupported rather than inherit another mechanism's claim.

**Boundary**: This clarifies ownership and future mechanism obligations without changing the minimal v0 public schema, deterministic identity, package format, current Agent prompts, current settlement behavior, or current adapter acceptance. It does not prove that a recent real whole-book artifact is convertible; that requires a separate artifact-specific audit/export against the source EPUB, `BookDocument`, and settled producer data.

**Why this path won**: A small stable semantic handoff preserves the only information the mechanism uniquely knows while letting deterministic code own everything derivable from the source or profile. This keeps future mechanism redesign free to change internal architecture without changing the public Pack or teaching the exporter every private ontology.
